...
@Component("autoEosBlock")
public class AutoEosBlock {

    *//**
     * 日志
     *//*
    private static final Logger logger = LoggerFactory.getLogger(AutoEosBlock.class);
		...
		public void run() {
            ...
            while (true) {
                ...
                List<SystemCoinType> collect = coinTypes.stream().filter(p -> 
                		...
                		//eos类
                		&& p.getCoinType().equals(SystemCoinSortEnum.EOS.getCode()) 
                ...
                try {
                    blockService.syncEosBlock(collect);
                    
				} catch (Exception e) {
					logger.error("autoEosBlock执行异常",e);
				}
            	try {
            		Thread.sleep(30000);
