...
public class WalletUtils {
	...
	//充值冻结增加
	public final static int depositFrozen_add = 6;
	...
	public Boolean change(Integer userId,Integer coinId , int businessType ,BigDecimal amount) throws BCException {
		...
		while (true ) {
			...
			//logger.info("用户钱包before："+wallet.toString() + "amount:"+amount.toString());
			if(wallet == null) {
			...
			case frozen_to_total:
				...
				wallet.setFrozen(MathUtils.sub(wallet.getFrozen(), amount));
				break;
			case depositFrozen_add:
				wallet.setDepositFrozen(MathUtils.add(wallet.getDepositFrozen(), amount));
				wallet.setDepositFrozenTotal(MathUtils.add(wallet.getDepositFrozenTotal(), amount));
				break;
			default:
				return false;
			}
			//logger.info("用户钱包after："+wallet.toString() + "amount:"+amount.toString());
