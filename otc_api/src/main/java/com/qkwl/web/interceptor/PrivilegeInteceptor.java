...
public class PrivilegeInteceptor implements HandlerInterceptor {
    ...
    private boolean preHandle20(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
        ...
        if(arg2 instanceof HandlerMethod) {
            ...
            if(annotation != null) {
            	if(checkRepeatSubmit(request, token)) {
                	logger.info("前台获取token为空----"+token);
                    ReturnResult rst = ReturnResult.FAILUER("重复提交");
...
//        }
    ...
    private boolean checkRepeatSubmit(HttpServletRequest httpServletRequest, String token) {
    	
    	try {
            ServletRequestAttributes attributes = (ServletRequestAttributes) RequestContextHolder.getRequestAttributes();
            HttpServletRequest request = attributes.getRequest();
            String key = token + "-" + request.getServletPath();
            logger.info("checkRepeatSubmit key = " + key);
            if (redisHelper.get(key) == null) {// 如果缓存中有这个url视为重复提交
                redisHelper.setRedisData(key, token, 2);
