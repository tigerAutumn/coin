    ...
    public static String getToken(HttpServletRequest request,RedisHelper redisHelper) {
        ...
        if (!StringUtils.isEmpty(token)) {
            ...
            if (StringUtils.isEmpty(json)) {
                throw new LoginException(ReturnResult.FAULURE_USER_NOT_LOGIN,I18NUtils.getString("PermissionCheckUtil.1"));
            }else{
                try {
                    ...
                    if (Utils.getTimestamp().getTime() / 1000 - obj.getLastActiveDateTime() > Constant.EXPIRETIME) {
                        ...
                        throw new LoginException(ReturnResult.FAULURE_USER_NOT_LOGIN, I18NUtils.getString("PermissionCheckUtil.3"));
                    }
                    Object resultStr = obj.getExtObject();
                    if (resultStr == null) {
                        logger.info("当前从redis获取token不为空且ExtObject为空----" + token);
                        throw new LoginException(ReturnResult.FAULURE_USER_NOT_LOGIN, I18NUtils.getString("PermissionCheckUtil.5"));
                    }
                }catch (Exception ex){
                    throw new LoginException(ReturnResult.FAULURE_USER_NOT_LOGIN,I18NUtils.getString("PermissionCheckUtil.1"));
                }
