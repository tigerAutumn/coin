...
public class AutoCache {
    ...
    private void initArticles() {
        ...
        for (FSystemLan fSystemLan : systemLans) {
            ...
            for (FArticleType fArticleType : farticletypes) {
                ...
                List<FArticle> farticles = redisHelper.getArticles(fSystemLan.getFid() , 2, fArticleType.getFid(), 1, 0);
    ...
    private void initSystemCoinType() {
        List<SystemCoinType> coinTypes = redisHelper.getCoinTypeListAll();
        if (coinTypes == null) {
            return;
        }
    ...
    public SystemTradeType getSystemTradeType(Integer tradeId) {
        return SystemTradeTypeJob.SystemTradeTypeMap.get(tradeId);
    }
}
