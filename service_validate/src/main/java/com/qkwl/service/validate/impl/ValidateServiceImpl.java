package com.qkwl.service.validate.impl;
...
import com.qkwl.service.validate.dao.ValidateEmailMapper;
import com.qkwl.service.validate.dao.ValidateSmsMapper;
import com.qkwl.service.validate.model.*;
import com.qkwl.service.validate.util.IOCloseUtils;
import com.qkwl.service.validate.util.JobUtils;
import okhttp3.*;
...
public class ValidateServiceImpl {
    ...
    public static void main(String args[]) {
        /**
         ...
         va.setAccessKey("N9628105");
         va.setSecretKey("zLsVkqu5p");
         va.setUrl("http://smssh1.253.com/msg/send/json");

         ValidateSmsDO vs = new ValidateSmsDO();
         vs.setContent("【热币网】您好，您的账号于2018-03-20使用了新的IP地址登陆，若非本人操作，请尽快联系客服冻结账号以确保账户安全。");
         vs.setPhone("15823488981");
         send253NormalSMS(va,vs); **/
    ...
    public Boolean updateSend(ValidateSendDTO vs) {
        ...
        if (vs.getSendType().equals(SendTypeEnum.EMAIL.getCode())) {
            ...
            email.setTitle(platform.getName());
            ...
            BusinessTypeEnum businessType = BusinessTypeEnum.getBusinessTypeByCode(vs.getBusinessType());
            sendSms.setBusinessType(businessType.getType());
            sendSms.setLang(LocaleEnum.getNameByCode(vs.getLanguageType()));
            sendSms.setPhone(vs.getPhone());
            sendSms.setPlatform("hotcoin");
            Map<String, Object> map = new HashMap<>();
            switch (businessType) {
			case SMS_WEB_REGISTER: case SMS_APP_REGISTER: case SMS_MODIFY_LOGIN_PASSWORD: case SMS_MODIFY_TRADE_PASSWORD:
			case SMS_FIND_PHONE_PASSWORD: case SMS_FIND_TRADE_PASSWORD: case SMS_MODIFY_GOOGLE: case SMS_MODIFY_SAFETY:
			case SMS_BING_MOBILE: case SMS_SEND_CODE: case SMS_COIN_WITHDRAW: case SMS_OPEN_SECONDARY_VERIFICATION:
			case SMS_CLOSE_SECONDARY_VERIFICATION: case SMS_SECONDARY_VERIFICATION:case SMS_SET_ANTI_PHISHING_CODE: 
			case SMS_MODIFY_ANTI_PHISHING_CODE: case CODE_LOGIN:case UPDATE_API_KEY:
				map.put("code", vs.getCode());
				break;
			case SMS_NEW_IP_LOGIN:
				map.put("mobile", vs.getPhone());
				SimpleDateFormat formatter1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				map.put("time", formatter1.format(new Date()));
				break;
			case SMS_NOTIFY_SUCC: case SMS_NOTIFY_FAIL: case SMS_ACCOUNT_LOCKED:
				map.put("mobile", vs.getPhone());
				break;
			case SMS_WITHDRAW_TO_ACCOUNT: case SMS_CHARGE_TO_ACCOUNT: case SMS_C2C_BUY: case SMS_C2C_SELL:
				map.put("mobile", vs.getPhone());
				SimpleDateFormat formatter2 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				map.put("time", formatter2.format(new Date()));
				map.put("coinCount", vs.getAmount());
				map.put("coinName", vs.getCoin());
				break;
			case SMS_ADD_PAYMENT_METHED: case SMS_UPDATE_PAYMENT_METHED:
				map.put("mobile", vs.getPhone());
				map.put("payment", vs.getPayment());
				map.put("account", vs.getAccount());
				break;
			default:
				// 查询模板信息
	        	ValidateTemplateDO template = jobUtils.getValidateTemplate(vs.getPlatformType(), vs.getSendType(),
			...
			sendSms.setParams(map);
            boolean send = sendSMS(sendSms);
            return send;
        }
    }
    
    private boolean sendSMS(SendSms sendSms) {
    	try {
	    	String url = noticeProperties.getUrl();
    ...
    private String getSendContent(String content, String params, ValidatePlatformDO platform, ValidateSendDTO vs) {
        ...
        for (String aParamArray : paramArray) {
            ...
            if (paramType.equals(ParameterTypeEnum.ANTI_PHISHING_CODE.getCode())) {
                content = content.replaceAll("#antiPhishingCode#", vs.getAntiPhishingCode());
            }
        }
        return content;
    }
    ...
    private boolean send253NormalSMS(ValidateAccountDO va, ValidateSmsDO vs) {
        ...
        try {
            ...
            return "1".equals(jsonObj.getString("code"));
        } catch (IOException e) {
            logger.error("send253NormalSMS fail->{}", e);
        }
        return false;
    }
...
//    }
     ...
     */
    private static boolean send253VoiceSms(ValidateAccountDO va, ValidateSmsDO vs) {
    ...
    private boolean send253InternationalSms(ValidateAccountDO va, ValidateSmsDO vs) {
        ...
        try {
            ...
            connection.disconnect();
            // 输出结果
            if (org.springframework.util.StringUtils.isEmpty(inputline)) {
                return false;
            }
            String[] result = inputline.split(",");
            logger.info(vs.getContent());
            logger.info("ak :" + va.getAccessKey());
            logger.info("sk :" + va.getSecretKey());
            if (result[1] != null && !result[1].equals("0")) {
                //logger.info("send253InternationalSms URL :{}",sb.toString());
                logger.info("send253InternationalSms fail, phone:{}, {}", vs.getPhone(), inputline);
                return false;
            }
        } catch (Exception e) {
            logger.error("send253InternationalSms failed, info:" + JSON.toJSONString(vs), e);
            return false;
        } finally {
            IOCloseUtils.closeIOStream(in);
        }
