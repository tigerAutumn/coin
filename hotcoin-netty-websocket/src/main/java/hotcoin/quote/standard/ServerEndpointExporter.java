...
import hotcoin.quote.exception.DeploymentException;
import hotcoin.quote.pojo.PojoEndpointServer;
import hotcoin.quote.pojo.PojoMethodMapping;
...
import hotcoin.quote.annotation.ServerEndpoint;
    ...
    private void init() {
        for (Map.Entry<InetSocketAddress, WebsocketServer> entry : addressWebsocketServerMap.entrySet()) {
            ...
            try {
                ...
                log.error("websocket [" + entry.getKey() + "] init fail", e);
            }
        }
    }
