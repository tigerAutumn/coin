...
public class UserLinksComponent {
    ...
    /**
     * 用户订阅币对保存,目前场景不需要使用克隆对象处理,使用非线程安全提高性能
     *
     * @param sessionId
    ...
    /**
     * 取消币对订阅 目前场景不需要使用克隆对象处理,使用非线程安全提高性能
     *
     * @param sessionId
     * @param pair
     */
    public void removeUserSubPair(String sessionId, String pair) {
        ...
        ConcurrentSkipListSet linkedSet = userSubPairsCacheMap.get(sessionId);
        if (linkedSet != null && !linkedSet.isEmpty()) {
