...
import hotcoin.quote.model.wsquote.TradeAreaTickersVo;
import hotcoin.quote.model.wsquote.TradeChangeTickersVo;
import hotcoin.quote.model.wsquote.TradeTickersDTO;
...
public class TickersComponent {
     ...
     * @return
     */
    public Map<String, List<TradeChangeTickersVo>> getChangeRateSortTickersCompare2Cache(List<TradeTickersDTO> list, int subNum) {
        if (MapUtils.isEmpty(cacheTickerMap)) {
            cacheAllTickers4Redis();
        }
    ...
    private TradeTickersDTO convertTickers(Integer tradeId, SystemTradeType systemTradeType, TickerData tickerData, String sellSymbol, String buySymbol) {
        ...
        // 获取折合cny价格
        String cnyPrice = tickerPriceCalcComponent.getCnyPrice(tradeId, last, buySymbol);
    ...
    public Map<String, List<TradeAreaTickersVo>> getArea4PushData(List<TradeTickersDTO> list) {
        ...
        for (TradeTickersDTO item : list) {
            ...
            String area = redisComponent.getAreaByTradeId(tradeId);
            if (area.equals(TickersConstant.TICKERS_BTC_AREA)) {
                btcList.add(convertDTO2AreaTickersVo(item));
            } else if (area.equals(TickersConstant.TICKERS_ETH_AREA)) {
                ethList.add(convertDTO2AreaTickersVo(item));
            } else if (area.equals(TickersConstant.TICKERS_GAVC_AREA)) {
                gavcList.add(convertDTO2AreaTickersVo(item));
            } else if (area.equals(TickersConstant.TICKERS_USDT_AREA)) {
                usdtList.add(convertDTO2AreaTickersVo(item));
            } else if (area.equals(TickersConstant.TICKERS_INNOVATE_AREA)) {
    ...
    private List<TradeChangeTickersVo> convertDTO2ChangeTickersVo(List<TradeTickersDTO> data) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            vo.setBuySymbol(item.getBuySymbol());
            ...
            vo.setSellSymbol(item.getSellSymbol());
