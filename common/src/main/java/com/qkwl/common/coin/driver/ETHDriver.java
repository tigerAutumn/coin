...
public class ETHDriver implements CoinDriver {
    ...
    public ETHDriver(String walletLink,String chainLink, String pass, Integer coinSort, String sendAccount, String contractAccount, int contractWei) {
	...
	public ReturnResult sendToAddress(String to, BigDecimal amount,BigDecimal fee, String nonce, String gasPrice, String gas ,String memo) {
		...
		try {
        	...
        	if(resultJson.containsKey("result")) {
        		...
        		}else if(message.indexOf("invalid argument") != -1){
        			return ReturnResult.FAILUER(403,"参数错误，用户提现地址错误或钱包不够钱了");
        		}else if(message.indexOf("invalid character") != -1) {
        			return ReturnResult.FAILUER(403,"提现失败，未知错误，请重新审核");
        		}else if(message.indexOf("nonce too low") != -1) {
        			return ReturnResult.FAILUER(409,"nonce too low，请重新审核");
        		}
        		
        		//{"id":1,"jsonrpc":"2.0","error":{"code":-32000,"message":"nonce too low"}}
        		//{"id":1,"jsonrpc":"2.0","error":{"code":-32000,"message":"replacement transaction underpriced"}}    由于还有转账还没打包，获取链上nonce在提交会出现重复的情况
        		//{"jsonrpc":"2.0","error":{"code":-32600,"message":"invalid character '\\x01' in string literal"}}
        		//{"id":1,"jsonrpc":"2.0","error":{"code":-32602,"message":"invalid argument 0: json: cannot unmarshal hex string of odd length into Go value of type hexutil.Bytes"}
        		//{"jsonrpc":"2.0","id":1,"error":{"code":-32602,"message":"invalid argument 0: hex string has length 8, want 40 for common.Address"}}
