...
public class Placeholder {
    ...
    public static String replace(String source,StrLookup<?> variableResolver ,String prefix, String suffix,boolean enableSubstitutionInVariables){
        //StrSubstitutor不是线程安全的类
        StrSubstitutor strSubstitutor = new StrSubstitutor(variableResolver, prefix, suffix,'0');
        //是否在变量名称中进行替换
        strSubstitutor.setEnableSubstitutionInVariables(enableSubstitutionInVariables);
        return strSubstitutor.replace(source);
    }
    ...
    public static void main(String[] args) {
    	...
    	System.out.println(replace("你的用户名是${123},密码是${password}", variableResolver, "${", "}", false));
	}
