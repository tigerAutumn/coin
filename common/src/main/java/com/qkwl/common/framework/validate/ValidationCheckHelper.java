...
/**
 * 有效性验证工具类 Created by ZKF on 2017/7/5.
 */
public class ValidationCheckHelper {

	private final static Logger logger = LoggerFactory.getLogger(ValidationCheckHelper.class);
	...
	public Result getPhoneCodeCheck(String areaCode, String phone, String code, Integer messageType, String ip, Integer platform) {
		logger.info("getPhoneCodeCheck areaCode:{},phone:{},code:{},messageType:{},ip:{},platform:{}", areaCode, phone, code, messageType, ip, platform);
		Integer type = LimitTypeEnum.PhoneCode.getCode();
		Boolean checkResult = limitHelper.checkLimit(ip, type);
		logger.info("ip:{},type:{},==checkResult:{}", ip, type, checkResult);
		if (!checkResult) {
			throw new BizException(ErrorCodeEnum.PHONE_LIMIT_BEYOND_ERROR);
		}
		...
		if (!mobilValidate) {
			...
			throw new BizException(ErrorCodeEnum.PHONE_LIMIT_COUNT_ERROR, limitCount);
		}
	...
	public Result getEmailCodeCheck(String email, String code, Integer messageType, String ip, Integer platform) {
		...
		Boolean checkResult = limitHelper.checkLimit(ip, type);
		if (!checkResult) {
			throw new BizException(ErrorCodeEnum.EMAIL_LIMIT_BEYOND_ERROR);
		}
		boolean mailValidate = validateHelper.mailCodeValidate(email, platform, messageType, code);

		if (!mailValidate) {
			Integer limitCount = limitHelper.updateLimit(ip, type);
			throw new BizException(ErrorCodeEnum.EMAIL_LIMIT_COUNT_ERROR, limitCount);
		}
	 ...
	 * @return 服务端返回消息
	 */
	public Result getCapitalCheck(FUser user, String phoneCode, Integer messageType, String googleCode, String tradePassword, String ip, Integer platform) {
	 ...
	 * @return 服务端返回消息
	 */
	public Result getChangeCheck(FUser user, String phoneCode, Integer messageType, String googleCode, String ip, Integer platform) {
	 ...
	 * @return 服务端返回消息
	 */
	public Result getChangeCheckOnlyGoogle(FUser user, String phoneCode, Integer messageType, String googleCode, String ip, Integer platform) {
		Result result = Result.success();
		if (user.getFgooglebind()) {
			result = getGoogleCodeCheck(user.getFgoogleauthenticator(), googleCode, ip);
			if (!result.getSuccess()) {
		...
		// if (user.getFistelephonebind()) {
		// result = getPhoneCodeCheck(user.getFareacode(), user.getFtelephone(),
		// phoneCode, messageType, ip, platform);
		// if (!result.getSuccess()) {
		// return result;
		// }
		// }
	 ...
	 * @return 服务端返回消息
	 */
	public Result getCapitalCombinationCheck(FUser user, String phoneCode, String emailCode, String googleCode, String tradePassword, String ip, Integer platform) {
		Result result = getTradePasswordCheck(user.getFtradepassword(), tradePassword, ip);
		if (!result.getSuccess()) {
			return result;
		}
		...
		if (user.isOpenPhoneValidate()) {
			result = getPhoneCodeCheck(user.getFareacode(), user.getFtelephone(), phoneCode, BusinessTypeEnum.SMS_COIN_WITHDRAW.getCode(), ip, platform);
			if (!result.getSuccess()) {
				return result;
			}
		}
		if (user.isOpenEmailValidate()) {
			result = getEmailCodeCheck(user.getFemail(), emailCode, BusinessTypeEnum.EMAIL_SEND_CODE.getCode(), ip, platform);
			if (!result.getSuccess()) {
				return result;
			}
		}
