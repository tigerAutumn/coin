...
public class ValidateHelper {
     ...
     * 新版发送通知短信调用
     */
    private boolean sendBaseSms(ValidateSendDTO send) {
        // MQ
        ...
        validateProducer.sendAsync(message, new SendCallback() {
            ...
            public void onException(OnExceptionContext context) {
            	logger.error("SendSMS mq send failed,{}",context.getException().fillInStackTrace());
            }
        });
        return true;
    }
    ...
    public boolean smsValidateCode(Integer uid, String areaCode, String phone, Integer sendType,
                                   Integer platform, Integer businessType, Integer languageType)  {
        ...
        if (obj != null) {
            ...
            if ((now - lastActive) < ValidataeConstant.MSG_SEND_TIME) {
            	 throw new BizException(ErrorCodeEnum.SEND_SMS_FREQUENTLY);
            }
        }
        //判断短信类型
    ...
    public boolean smsPriceClock(FUser user, Integer sendType, Integer platform, Integer businessType, Integer languageType,
        ...
        if (StringUtils.isEmpty(coinId) || StringUtils.isEmpty(type) || user == null || sendType == null || platform == null
            ...
            logger.warn("smsPriceClock params is null, user:{}, sendType:{}, platform:{}, businessType:{}, languageType:{}" +
    ...
    public boolean smsRiskManage(String username, String phone, Integer platform, Integer businessType,
        ...
        if (StringUtils.isEmpty(username) || StringUtils.isEmpty(phone) || StringUtils.isEmpty(type) || StringUtils.isEmpty(coin)
            ...
            logger.warn("smsRiskManage params is null, username:{}, phone:{}, platform:{}, businessType:{}, type:{}, " +
    ...
    public boolean smsSensitiveInfo(String areaCode, String phone, Integer languageType,
                                    Integer platform, Integer businessType)  {
        ...
        if (StringUtils.isEmpty(areaCode)) {
            ...
            throw new BizException(ErrorCodeEnum.AREA_CODE_EMPTY);
        }
        if (!areaCode.equals("86")) {
            if (phone.charAt(0) == '0') {
                phone = phone.substring(1, phone.length());
     ...
     * 发送充币到账短信
     */
    public boolean smsCoinToAccount(String areaCode, String phone, Integer platform, Integer businessType,
                                   BigDecimal amount, String coinName, Integer languageType) throws BCException {
        if (StringUtils.isEmpty(areaCode) || StringUtils.isEmpty(phone) || platform == null
        		|| businessType == null || amount == null || StringUtils.isEmpty(coinName) || languageType == null) {
            logger.warn("smsCoinToAccount params is null, areaCode:{}, phone:{}, platform:{}, businessType:{}, amount:{}, coinName:{}, languageType:{}"
                    , areaCode, phone, platform, businessType, amount, coinName, languageType);
            return false;
        }
        Integer sendType = SendTypeEnum.SMS_TEXT.getCode();
    ...
    public Boolean validateSmsCode(String areaCode, String phone,
        ...
        if (StringUtils.isEmpty(phone) || code == null || platform == null || businessType == null) {
            logger.warn("validateSmsCode params is null, areaCode:{}, phone:{}, code:{}, platform:{}, businessType:{}"
                    , areaCode, phone, code, platform, businessType);
            return false;
        }
        String token = RedisConstant.VALIDATE_KEY + phone + "_" + platform + "_" + businessType;
     ...
     */
    public Boolean mailSendContent(String email, PlatformEnum platform, LocaleEnum locale, BusinessTypeEnum businessType, String ip, FUser user,String antiPhishingCode) {
        ...
        sendDTO.setIp(ip);
        sendDTO.setAntiPhishingCode(antiPhishingCode);
        // 投递redis
     ...
     * @return true 成功，false 失败
     */
    public Boolean mailSendCode(String email, PlatformEnum platform, LocaleEnum languageType, String ip,String antiPhishingCode) {
        if (StringUtils.isEmpty(email) || StringUtils.isEmpty(ip) || platform == null
                || languageType == null) {
     ...
     * @return true 成功，false 失败
     */
    public Boolean mailSendCode(String email, PlatformEnum platform,int businessType, LocaleEnum languageType, String ip,String antiPhishingCode) {
        if (StringUtils.isEmpty(email) || StringUtils.isEmpty(ip) || platform == null
                || languageType == null) {
            logger.warn("mailSendCode params is null, email:{}, platform:{}, businessType:{}, languageType:{}, ip:{}",
                    email, platform, businessType, languageType, ip);
            return false;
        }
        // 判断是否已发送
        String token = RedisConstant.VALIDATE_KEY + businessType + "_"+ platform.getCode() + "_" + email;
        RedisObject json = redisHelper.getRedisObject(token);
        if (json != null) {
        	throw new BizException(ErrorCodeEnum.SEND_EMAIL_MAX_1_IN_TIME,ValidataeConstant.EMAIL_CODE_OUT_TIME/60);
        }
        // 数据组装
        String code = Utils.randomInteger(6);
        String uuid = GUIDUtils.getGUIDString();
    ...
    public Boolean mailSendPriceClock(String email, PlatformEnum platform, LocaleEnum locale, BusinessTypeEnum businessType,
                                      String coinName, String type, BigDecimal currPrice, BigDecimal settingPrice, Integer uid,String antiPhishingCode) {
    ...
    public Boolean mailSendFinanceBalance(String email, PlatformEnum platform, LocaleEnum locale, BusinessTypeEnum businessType,
                                      String yesterdayStr, String customUrl, String account,String antiPhishingCode) {
    ...
    private Boolean mailSendBase(ValidateSendDTO sendDTO) {
        ...
        validateProducer.sendAsync(message, new SendCallback() {
            ...
            @Override
            public void onException(OnExceptionContext context) {
                logger.error("SendEmail mq send failed",context.getException());
            }
        });
        return true;
    }
     ...
     * 发送充币到账短信
     */
    public boolean smsPaymentMethod(String areaCode, String phone, Integer platform, Integer businessType,
                                   String payment, String account, Integer languageType) throws BCException {
        if (StringUtils.isEmpty(areaCode) || StringUtils.isEmpty(phone) || platform == null
        		|| businessType == null || StringUtils.isEmpty(payment) || StringUtils.isEmpty(account) || languageType == null) {
            logger.warn("smsCoinToAccount params is null, areaCode:{}, phone:{}, platform:{}, businessType:{}, payment:{}, account:{}, languageType:{}"
                    , areaCode, phone, platform, businessType, payment, account, languageType);
            return false;
        }
        Integer sendType = SendTypeEnum.SMS_TEXT.getCode();
        ...
        send.setSendType(sendType);
        send.setLanguageType(languageType);
        send.setPayment(payment);
        send.setAccount(account);
