...
import com.qkwl.common.dto.Enum.otc.OtcCurrencyStatusEnum;
...
import com.qkwl.common.dto.lever.SystemLeverArgs;
...
import com.qkwl.common.dto.otc.ListPaymentResp;
...
public class RedisHelper {
	 ...
	 */
	public List<FArticle> getArticles(int lanId ,int type, int Articlestype, int currentPage, int agentId) {
		String articles = memCache.get(RedisConstant.WEB_ARTICLE_KET + lanId + "_" + type + "_" + agentId + "_" + Articlestype + "_" + currentPage);
	 ...
	 * @return FArticle
	 */
	public FArticle getArticleById(int lanId , int type, int id, int agentId) {
		String articles = memCache.get(RedisConstant.WEB_ARTICLEDETAIL_KET + lanId + "_" + type + "_" + agentId + "_" + id);
		if (StringUtils.isEmpty(articles)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(articles);
	 ...
	 * @return FArticle
	 */
	public FArticle getArticleById(int id) {
		String articles = memCache.get(RedisConstant.WEB_ARTICLEDETAIL_KET + id);
		if (StringUtils.isEmpty(articles)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(articles);
	 ...
	 * @return int
	 */
	public int getArticlesPageCont(int lanId ,int type, int articlestype, int agentId) {
		String articlePageCont = memCache.get(RedisConstant.WEB_ARTICLECOUNT_KET +lanId +"_" + type + "_" + agentId + "_" + articlestype);
	 ...
	 * @return FAbout
	 */
	public VersionUpgradeInfo getAppVersion(int type) {
		String aersionUpgradeInfos = memCache.get(RedisConstant.APP_VERSION_KEY + type);
		if (StringUtils.isEmpty(aersionUpgradeInfos)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(aersionUpgradeInfos);
		VersionUpgradeInfo aersionUpgradeInfo = JSON.parseObject(obj.get("extObject").toString(), VersionUpgradeInfo.class);
		return aersionUpgradeInfo;
	}
	 ...
	 * @return List
	 */
	public List<SystemCoinType> getOtcCoinTypeListSystem(){
		String coins = memCache.get(RedisConstant.COIN_LIST_KEY);
		if (StringUtils.isEmpty(coins)) {
			return null;
		}
	...
	public SystemCoinType getCoinTypeShortName(String shortName) {
		...
		for (SystemCoinType systemCoinType : coinList) {
			if (systemCoinType.getShortName().equals(shortName)) {
				return systemCoinType;
			}
		}
		return null;
	 ...
	 * @return SystemCoinType
	 */
	public SystemCoinType getCoinTypeByCoinSort(Integer coinType) {
		SystemCoinType object = getCoinType(coinType);
		//比特币类返会每个币本身
		if(object.getCoinType().equals(SystemCoinSortEnum.ETH.getCode())) {
			return getCoinTypeShortName("ETH");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.VCC.getCode())) {
			return getCoinTypeShortName("VCC");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.MOAC.getCode())) {
			return getCoinTypeShortName("MOAC");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.FOD.getCode())) {
			return getCoinTypeShortName("FOD");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.ETC.getCode())) {
			return getCoinTypeShortName("ETC");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.BWT.getCode())) {
			return getCoinTypeShortName("BWT");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.NEO.getCode())) {
			return getCoinTypeShortName("NEO");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.CET.getCode())) {
			return getCoinTypeShortName("CET");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.SERO.getCode())) {
			return getCoinTypeShortName("SERO");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.TRX.getCode())) {
			return getCoinTypeShortName("TRX");
		}else if(object.getCoinType().equals(SystemCoinSortEnum.ICS.getCode())){
            return getCoinTypeShortName("ICS");
        } else if(object.getCoinType().equals(SystemCoinSortEnum.MIC.getCode())){
            return getCoinTypeShortName("MIC");
        }else if(object.getCoinType().equals(SystemCoinSortEnum.ONT.getCode())){
            return getCoinTypeShortName("ONT");
        }
		return object;
	}
	 ...
	 * @return Map<Integer, String>
	 */
	public Map<Integer, String> getCoinTypeNameSymbolMap(){
		Map<Integer,String> coinMap = new TreeMap<Integer,String>(new Comparator<Integer>() {
			public int compare(Integer o1, Integer o2) {
				// 降序排序
				return o1.compareTo(o2);
		...
		List<SystemCoinType> coinTypes = getCoinTypeListSystem();
		for (SystemCoinType coinType : coinTypes){
			coinMap.put(coinType.getId(),coinType.getName() + " " + coinType.getShortName());
		}
		return coinMap;
	}
	 ...
	 * @return Map<Integer, String>
	 */
	public Map<Integer, String> getOtcCoinTypeNameMap(){
		Map<Integer,String> coinMap = new TreeMap<Integer,String>(new Comparator<Integer>() {
			public int compare(Integer o1, Integer o2) {
				// 降序排序
				return o1.compareTo(o2);
		...
		List<SystemCoinType> coinTypes = getOtcCoinTypeListSystem();
		for (SystemCoinType coinType : coinTypes){
			coinMap.put(coinType.getId(),coinType.getName());
		}
		return coinMap;
	...
	public Map<Integer, String> getCoinTypeCNYNameMap(){
		...
		SystemCoinType gavc = getCoinTypeShortName(Constant.CNY_COIN_NAME);
		if(gavc != null) {
			coinMap.put(gavc.getId(), gavc.getName());
		}
		return coinMap;
	}
	/**
	 ...
	 * @return List<SystemCoinType>
	 */
	public List<OtcCurrency> getOpenOtcCurrencyList(){
		List<OtcCurrency> currencyList = getAllCurrencyList();
		if(currencyList == null){
			return null;
		}
		
		List<OtcCurrency> collect = currencyList.stream()
				.filter(f -> 
				f.getStatus().equals(OtcCurrencyStatusEnum.NORMAL.getCode()))  //启用
	 ...
	 * @return List<SystemCoinType>
	 */
	public List<OtcPayment> getOpenOtcPaymentList(){
		List<OtcPayment> paymentList = getAllPaymentList();
		if(paymentList == null){
			return null;
		}
		
		List<OtcPayment> collect = paymentList.stream()
				.filter(f -> 
				f.getStatus().equals(OtcPaymentStatusEnum.Normal.getCode()))  //启用
				.collect(Collectors.toList());
		return collect;
	}
	 ...
	 * @return List<SystemCoinType>
	 */
	public List<SystemCoinType> getOpenOtcCoinTypeList(){
		List<SystemCoinType> coinList = getCoinTypeListAll();
		if(coinList == null){
			return null;
		}
		
		List<SystemCoinType> collect = coinList.stream()
				.filter(f -> 
				f.getStatus().equals(SystemCoinStatusEnum.NORMAL.getCode())  //启用
				&& f.isOpenOtc()  //开放otc
				&& f.getType().equals(SystemCoinTypeEnum.COIN.getCode()))  //虚拟币
				.collect(Collectors.toList());
		return collect;
	}
	 ...
	 * @return
	 */
	public List<SystemTradeType> getTradeTypeShareByTradeIds(JSONArray parseArray,Integer agentId){
		String coins = memCache.get(RedisConstant.TRADE_LIST_KEY + "_" + agentId);
		if (StringUtils.isEmpty(coins)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(coins);
		List<SystemTradeType> tradeList = JSONArray.parseArray(obj.getJSONArray("extObject").toString(), SystemTradeType.class);
		List<SystemTradeType> newCoinList = new ArrayList<>();
		
		for(Object val : parseArray) {
			if(val != null) {
				Integer tradeId = Integer.parseInt(val.toString());
				for (SystemTradeType systemTradeType : tradeList) {
					if (!systemTradeType.getStatus().equals(SystemTradeStatusEnum.ABNORMAL.getCode()) 
							&& systemTradeType.getId().equals(tradeId)) {
						newCoinList.add(systemTradeType);
					}
				}
			}
	...
	/**
	 * 获取未禁用的交易列表(根据交易类型包含创新区)
	 * @param agentId 劵商ID
	 * @return
	 */
	public List<SystemTradeType> getTradeTypeSortAndInnovation(Integer type,Integer agentId){
		String coins = memCache.get(RedisConstant.TRADE_LIST_KEY + "_" + agentId);
		if (StringUtils.isEmpty(coins)) {
			return null;
		}
		...
		for (SystemTradeType systemTradeType : coinList) {
			if (!systemTradeType.getStatus().equals(SystemTradeStatusEnum.ABNORMAL.getCode()) && 
					(systemTradeType.getType().equals(type) || systemTradeType.getType().equals(SystemTradeTypeEnum.INNOVATION_AREA.getCode()))) {
				newCoinList.add(systemTradeType);
			}
		}
		return newCoinList;
	 ...
	 * 获取交易币种规则
	 */
	public List<SystemTradeTypeRule> getTradeTypeRules(Integer tradeId,String recType) {
		String rules = memCache.get(RedisConstant.TRADE_REC_RULES_KEY + tradeId +"_" + recType);
		logger.debug("rules:",JSON.toJSONString(rules));
		if (StringUtils.isEmpty(rules) ) {
			return null;
		}
		List<SystemTradeTypeRule> ruleList;
		JSONObject obj = JSON.parseObject(rules);
		ruleList = JSONArray.parseArray(obj.getJSONArray("extObject").toJSONString(),
				SystemTradeTypeRule.class);
		return ruleList;
	...
	public Map<Integer, Integer> getCoinIdToTradeId(Integer agentId) {
		...
		List<SystemTradeType> tradeTypes = getTradeTypeSort(SystemTradeTypeEnum.GAVC.getCode(),agentId);
		if (tradeTypes == null) {
	...
	public SystemTradeType getTradeTypeFromInnvationCoinId(Integer coinId, Integer agentId) {
		List<SystemTradeType> tradeTypes = getTradeTypeSort(SystemTradeTypeEnum.INNOVATION_AREA.getCode(), agentId);
		if (tradeTypes == null) {
			return null;
		}
		for (SystemTradeType tradeType : tradeTypes) {
			if(tradeType.getSellCoinId() == coinId) {
				return tradeType;
			}
		}
  ...
  public List<FFriendLink> getFFriendLinkList() {
    ...
    List<FFriendLink> friendLinks =
        JSON.parseArray(obj.get("extObject").toString(), FFriendLink.class);

    for (FFriendLink f : friendLinks) {
      f.setFname(getMultiLangMsg(f.getFname()));
      f.setFurl(getMultiLangMsg(f.getFurl()));
      f.setFdescription(getMultiLangMsg(f.getFdescription()));
    }
	...
	public boolean set(String key, String t, int seconds) {
		return memCache.set(key, t, seconds);
	}
	...
	public Long getIncrByKey(String key) {
		return memCache.incr(key);
	}
	
	public Long getDecrByKey(String key) {
		return memCache.decr(key);
	}

	public Long getIncrByKey(String key,Integer timeout) {
		return memCache.incr(key,timeout);
	}
	 ...
	 * @return
	 */
	public TickerData getTickerData(int tradeId) {
		String result = memCache.get(RedisConstant.TICKERE_KEY + tradeId);
		if (result == null || "".equals(result)) {
	 ...
	 * @return List
	 */
	public List<OtcCurrency> getAllCurrencyList(){
		String currnency = memCache.get(RedisConstant.CURRENCY_LIST_KEY);
		if (StringUtils.isEmpty(currnency)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(currnency);
		return JSONArray.parseArray(obj.getJSONArray("extObject").toString(), OtcCurrency.class);
	}
	 ...
	 * @return List
	 */
	public List<OtcPayment> getAllPaymentList(){
		String payment = memCache.get(RedisConstant.PAYMENT_LIST_KEY);
		if (StringUtils.isEmpty(payment)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(payment);
		return JSONArray.parseArray(obj.getJSONArray("extObject").toString(), OtcPayment.class);
	}
	 ...
	 * @return List
	 */
	public List<HistoricVersion> getHistoricVersionList(){
		String versionListStr = memCache.get(RedisConstant.HISTORIC_VERSION);
		if (StringUtils.isEmpty(versionListStr)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(versionListStr);
		return JSONArray.parseArray(obj.getJSONArray("extObject").toString(), HistoricVersion.class);
	}

	  public String getMultiLangMsg(String code) {
	    Locale currentLocale = I18NUtils.getCurrentLocale();
	    String msg = getMultiLangMsg(code,I18NUtils.getCurrentLocale());
	    //logger.info("code:{},语言:{},msg:{},",code,currentLocale.toString(),msg);
	    return msg;
	  }

	  public String getMultiLangMsg(String code, Locale locale) {
	    String multiLangStr = memCache.get(RedisConstant.MULTI_LANG_KEY+code+"_"+locale.toString());
        if (StringUtils.isEmpty(multiLangStr)) {
            return "";
        }
        JSONObject obj = JSON.parseObject(multiLangStr);

        MultiLang multiLang = JSON.parseObject(obj.getJSONObject("extObject").toString(), MultiLang.class);
        return multiLang.getMsg();
	  }
	 ...
	 * @return
	 */
	public BigDecimal getLastPrice(Integer type, Integer sellCoinId) {
		String coins = memCache.get(RedisConstant.TRADE_LIST_KEY + "_" + Constant.BCAgentId);
		if (StringUtils.isEmpty(coins)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(coins);
		...
		Integer tradeId = 0;
		for (SystemTradeType systemTradeType : coinList) {
			if (systemTradeType.getType() == type && systemTradeType.getSellCoinId() == sellCoinId 
					&& systemTradeType.getStatus().equals(SystemTradeStatusEnum.NORMAL.getCode())) {
				tradeId = systemTradeType.getId();
			}
		}
		return getLastPrice(tradeId);
	}
	 ...
	 * @return
	 */
	public SystemLeverArgs getLeverArgs() {
		String payment = memCache.get(RedisConstant.LEVER_ARGS_KET);
		if (StringUtils.isEmpty(payment)) {
			return null;
		}
		JSONObject obj = JSON.parseObject(payment);
		return JSON.parseObject(obj.getString("extObject"), SystemLeverArgs.class);
	}
	
	public void expire(String keyname, int expireTime) {
		memCache.expire(keyname, expireTime);
	}
	
	
	public String getLastKlineJson(Integer tradeId, Integer stepid) {
        String klineStr = getRedisData(RedisConstant.LASTKLINE_KEY + tradeId + "_" + stepid);
        if (StringUtils.isBlank(klineStr)) {
        	klineStr="[["+System.currentTimeMillis()+",0,0,0,0,0]]";
        }
        return klineStr;
    }
    
    
	public FPeriod getLastKline(Integer tradeId, Integer stepid) {
    	FPeriod fPeriod=null;
    	String lastKlineJson = getLastKlineJson(tradeId, stepid); 
    	JSONArray parseArray = JSON.parseArray(lastKlineJson).getJSONArray(0);
    	if(parseArray.size()==6) {
    		fPeriod=new FPeriod();
    		fPeriod.setFtradeid(tradeId);
    		fPeriod.setFtime(parseArray.getSqlDate(0));
    		fPeriod.setFkai(parseArray.getBigDecimal(1));
    		fPeriod.setFgao(parseArray.getBigDecimal(2));
    		fPeriod.setFdi(parseArray.getBigDecimal(3));
    		fPeriod.setFshou(parseArray.getBigDecimal(4));
    		fPeriod.setFliang(parseArray.getBigDecimal(5));
    	}
        return fPeriod;
    }
