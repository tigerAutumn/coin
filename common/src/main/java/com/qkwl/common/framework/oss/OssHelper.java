package com.qkwl.common.framework.oss;
...
public class OssHelper {
	 ...
	 * @return
	 */
	public String uploadValidateFile(byte[] file,String fileName,String filePath) {
		String result;
		try {
			result = uploadOSS(getBucketBase(), "image/jpeg", new ByteArrayInputStream(file), file.length, fileName, filePath);
		} catch (Exception e) {
			return null;
		}
		if (result == null) {
	 ...
	 * @return
	 */
	public String uploadPrivateFile(byte[] file,String fileName,String filePath) {
		String result;
		try {
			result = uploadPrivateOSS(getBucketBase(), "image/jpeg", new ByteArrayInputStream(file), file.length, fileName, filePath);
		} catch (Exception e) {
			return null;
		}
		if (result == null) {
	 ...
	 * @return
	 */
	public String uploadPrivateOSS(String Bucket, String contentType, InputStream input, long fileSize, String fileName, String filePath) throws Exception {
		try {
			// 参数校验
			if (StringUtils.isEmpty(contentType) || StringUtils.isEmpty(filePath) || StringUtils.isEmpty(fileName)) {
				return null;
			...
			ObjectMetadata objectMeta = new ObjectMetadata();
			objectMeta.setContentLength(fileSize);
			objectMeta.setObjectAcl(CannedAccessControlList.Private);
			try {
				// 创建bucket
				client.createBucket(Bucket);
				client.setBucketAcl(Bucket, CannedAccessControlList.PublicRead);
