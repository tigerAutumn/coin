...
public interface IUserCapitalAccountService {
     ...
     * 1006: 您已绑定该银行卡，请勿重复绑定！<br/>
     */
    Result createOrUpdateBankInfoV1(UserBankinfoDTO userBankinfo);
    
    /**
     * 创建或修改支付方式
     * @param userBankinfo 银行信息DTO
     * @param otcPayment 支付方式
     * @return Result   返回结果<br/>
     * 200 : 成功
     * 1000: 提现银行未找到！<br/>
     * 1001: 修改失败！<br/>
     ...
     * 1006: 您已绑定该银行卡，请勿重复绑定！<br/>
     */
    Result createOrUpdatePayment(UserBankinfoDTO userBankinfo);
     ...
     * @return List
     */
    List<FUserBankinfoDTO> listBankInfo(Integer userId, Integer type,Integer status);
     ...
     * 设置默认银行卡
     * @param userId    用户ID
     * @param id 用户绑定银行卡 user_bankinfo id
     * @return Result
     * 1002 地址不存在
     */
    Result defaultBankInfo(Integer userId, Integer id);
}
