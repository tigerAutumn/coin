...
public class FODController extends JsonBaseController {
    ...
    @PassToken
    @ApiOperation("")
	@RequestMapping(value="/coin/fod/recharge",method = {RequestMethod.GET,RequestMethod.POST})
    public String ethRecharge(
        ...
        if ("".equals(txhash) || "".equals(to) || "".equals(value) || "".equals(sign) || "".equals(symbol)) {  //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
            return ReturnMsg(10001, I18NUtils.getString("FODController.5")); 
        }
        SystemCoinType coinType = redisHelper.getCoinTypeShortNameSystem(symbol.toUpperCase());
        if (coinType == null) {
            return ReturnMsg(10001, I18NUtils.getString("FODController.6")); 
        }
        ...
        if (coinType.getShortName().equals("FOD")) { 
            ...
            baseCoinType = redisHelper.getCoinTypeShortName("FOD"); 
        }
        if (baseCoinType == null) {
            return ReturnMsg(10001, I18NUtils.getString("FODController.9")); 
        }
        //验证签名
        try {
            ...
            CoinDriver coinDriver = new CoinDriverFactory.Builder(coinType.getCoinType(), coinType.getWalletLink(), coinType.getChainLink())
                    ...
                    .getDriver();
            if (coinDriver == null) {
                return ReturnMsg(10001, I18NUtils.getString("FODController.10")); 
            }
            signVerify = coinDriver.getETCSHA3(signVerify);
            if (!signVerify.equals(sign)) {
                return ReturnMsg(10001, I18NUtils.getString("FODController.11")); 
            }
        } catch (Exception e) {
            logger.error("fodRecharge sign error, symbol：{},e:{}", symbol,e); 
            return ReturnMsg(10001, I18NUtils.getString("FODController.13")); 
        }
        ...
        try {
            ...
            if (!result.getSuccess()) {
            	logger.error("/coin/fod/recharge error ------------------------->：{}",result.toString()); 
                return ReturnMsg(result.getCode(), I18NUtils.getString("FODController.16")); 
            }
        } catch (Exception e) {
            logger.error("FODRecharge 服务器错误，symbol：{},e:{}", symbol,e); 
            return ReturnMsg(10001, I18NUtils.getString("FODController.18")); 
        }

        return ReturnMsg(10000, I18NUtils.getString("FODController.19")); 
    }
