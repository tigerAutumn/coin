...
@Controller
@ApiIgnore
public class FrontAccountJsonController extends JsonBaseController {
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="/deposit/alipay_manual",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult alipayManual(
            ...
            @RequestParam(value = "ubank", required = true) int userBankId){
        ...
        if (coinType == null || !coinType.getIsRecharge()
            ...
            return ReturnResult.FAILUER(I18NUtils.getString("financial.err.1002")); 
        }
        ...
        if (moneyDecimal.compareTo(WebConstant.MINRECHARGECNY) < 0) {
            return ReturnResult.FAILUER(I18NUtils.getString("financial.err.1004", MathUtils.decimalFormat(WebConstant.MINRECHARGECNY))); 
        }
        // 最大充值金额
        if (moneyDecimal.compareTo(WebConstant.MAXRECHARGECNY) > 0) {
            return ReturnResult.FAILUER(I18NUtils.getString("financial.err.1005", MathUtils.decimalFormat(WebConstant.MAXRECHARGECNY))); 
        }
        ...
        if (result.getCode() > 200 && result.getCode() < 1000) {
            return ReturnResult.FAILUER(I18NUtils.getString("common.error." + result.getCode(), result.getData())); 
        } else if (!result.getSuccess()) {
            return ReturnResult.FAILUER(I18NUtils.getString("capital.cny.operation." + result.getCode(), result.getData())); 
        }
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="withdraw/cny_manual",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult withdrawBankSubmit(String tradePwd,
        ...
        if (withdrawBlank == null) {
            return ReturnResult.FAILUER(I18NUtils.getString("capital.bank.withdraw.com.1000")); 
        }

        if (withdrawBalance == null) {
            return ReturnResult.FAILUER(I18NUtils.getString("capital.bank.withdraw.com.1001")); 
        }
        ...
        if (withdrawBalance.compareTo(BigDecimal.ZERO) <= 0) {
            return ReturnResult.FAILUER(I18NUtils.getString("capital.bank.withdraw.com.1002")); 
        }
        ...
        try {
            ...
            if (result.getSuccess()) {
                return ReturnResult.SUCCESS(I18NUtils.getString("FrontAccountJsonController.24")); 
            } else {
                return ReturnResult.FAILUER(result.getCode() + result.getMsg());
            }
        } catch (Exception e) {
            logger.error("withdrawBankSubmit createBankOperationOrder:{}",e);
        }
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="/withdraw/coin_manual",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult withdrawBtcSubmit(
        ...
        try {
            ...
            if (result.getSuccess()) {
                ...
                return ReturnResult.FAILUER(I18NUtils.getString("capital.coin.withdraw.create." + result.getCode().toString(), result.getData().toString())); 
            } else {
                return ReturnResult.FAILUER(I18NUtils.getString("com.error." + result.getCode().toString(), result.getData().toString())); 
            }
        } catch (Exception e) {
            logger.error("withdrawBtcSubmit error,e:{}",e); 
        }
        return ReturnResult.FAILUER(I18NUtils.getString("com.error.10117")); 
    }
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="/withdraw/coin_cancel",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult cancelWithdrawBtc(@RequestParam Integer id){
        FUser user = getCurrentUserInfoByToken();
        String ip = HttpRequestUtils.getIPAddress();
        try {
            Result result = userCapitalService.cancleCoinOperationOrder(user.getFid(), id, ip);
            if (result.getSuccess()) {
                return ReturnResult.SUCCESS(I18NUtils.getString("capital.coin.withdraw.cancle." + result.getCode().toString())); 
            } else if (result.getCode().equals(Result.PARAM)) {
                logger.error("cancelWithdrawBtc {}", result.getMsg()); 
            } else if (result.getCode() < 10000) {
                return ReturnResult.FAILUER(I18NUtils.getString("capital.coin.withdraw.cancle." + result.getCode().toString(), result.getData().toString())); 
            } else {
                return ReturnResult.FAILUER(I18NUtils.getString("com.error." + result.getCode().toString(), result.getData().toString())); 
            }
        } catch (Exception e) {
            logger.error("cancelWithdrawBtc error,e:{}",e); 
        }
        return ReturnResult.FAILUER(I18NUtils.getString("com.error.10117")); 
    }
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="/withdraw/coin_address",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult getVirtualAddress(
        ...
        try {
            ...
            SystemCoinType coinType = redisHelper.getCoinType(symbol);
            SystemCoinInfo coinInfo = redisHelper.getCoinInfo(coinType.getId(), String.valueOf(getLanEnum().getCode()));
            ...
            if (result.getCode() == 1000) {
                return ReturnResult.FAILUER(I18NUtils.getString("financial.err.1003")); 
            }
            FUserVirtualAddressDTO data = (FUserVirtualAddressDTO) result.getData();
            if(coinInfo != null) {
                data.setRechargeWarnWord(coinInfo.getRechargeWarnWord());
            }else {
                data.setRechargeWarnWord("");

            }
            data.setIsUseMemo(coinType.getIsUseMemo());
            return ReturnResult.SUCCESS(200, data);
        } catch (Exception e) {
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="/deposit/getsysbankinfo",method = {RequestMethod.GET,RequestMethod.POST})
    ...
    @PassToken
    @ApiOperation("")
	@RequestMapping(value="/deposit/cny_deposit_json",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult cnyDeposit(
        ...
        Pagination<FWalletCapitalOperationDTO> page = new Pagination<>(currentPage,
                Constant.CapitalRecordPerPage, "/deposit/cny_deposit.html?symbol=" + symbol + "&");  //$NON-NLS-2$
    ...
    @PassToken
    @ApiOperation("")
	@RequestMapping(value="/withdraw/cny_withdraw_json",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult withdrawCny(
        ...
        String currentLocal =I18NUtils.getCurrentLang();
        ...
        Pagination<FWalletCapitalOperationDTO> page = new Pagination<FWalletCapitalOperationDTO>(currentPage,
                Constant.CapitalWithdrawPerPage, "/withdraw/cny_withdraw.html?symbol=" + symbol + "&");  //$NON-NLS-2$
        FWalletCapitalOperationDTO operation = new FWalletCapitalOperationDTO();
        operation.setFuid(fuser.getFid());
    ...
    @PassToken
    @ApiOperation("")
	@RequestMapping(value="/deposit/coin_deposit_json",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult rechargeBtc(@RequestParam(required = false, defaultValue = "0") Integer symbol) {
        SystemCoinType coinType = redisHelper.getCoinType(symbol);
        if (coinType == null || !coinType.getIsRecharge()
                || coinType.getStatus().equals(SystemCoinStatusEnum.ABNORMAL.getCode())
    ...
    @PassToken
    @ApiOperation("")
	@RequestMapping(value="/withdraw/coin_withdraw_json",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult withdrawBtc(
        ...
        Pagination<FVirtualCapitalOperationDTO> page = new Pagination<FVirtualCapitalOperationDTO>(1, Constant.webPageSize,
                "/withdraw/coin_withdraw_json?symbol" + symbol + "&");  //$NON-NLS-2$
        FVirtualCapitalOperationDTO operation = new FVirtualCapitalOperationDTO();
        operation.setFuid(user.getFid());
    ...
    @PassToken
    @ApiOperation("")
	@RequestMapping(value="/financial/record_json",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult record(
            ...
            @RequestParam(required = false, defaultValue = "2") Integer datetype){
        ...
        for (SystemCoinType systemCoinType : coinTypes) {
            String valueR = I18NUtils.getString("financial.account.recordselectr"); 
            if (!StringUtils.isEmpty(valueR)) {
                KeyValues keyValues = new KeyValues();
                keyValues.setKey(keyFormat + "?symbol=" + systemCoinType.getId() + "&type=1");  //$NON-NLS-2$
                keyValues.setValue(String.format(valueR, systemCoinType.getShortName()));
                filters.add(keyValues);
            }
            String valueW = I18NUtils.getString("financial.account.recordselectw"); 
            if (!StringUtils.isEmpty(valueR)) {
                KeyValues keyValues = new KeyValues();
                keyValues.setKey(keyFormat + "?symbol=" + systemCoinType.getId() + "&type=2");  //$NON-NLS-2$
        ...
        if (coinType.getType().equals(SystemCoinTypeEnum.CNY.getCode())) {
            ...
            Pagination<FWalletCapitalOperationDTO> page = new Pagination<>(currentPage,
                    ...
                    "/financial/record.html?symbol=" + symbol + "&type=" + type + "&datetype=" + datetype + "&begindate="  //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                            + begindate + "&enddate=" + enddate + "&");  //$NON-NLS-2$
            /*Pagination<FWalletCapitalOperationDTO> page = new Pagination<>(currentPage,
            ...
            Pagination<FVirtualCapitalOperationDTO> page = new Pagination<FVirtualCapitalOperationDTO>(currentPage,
                    Constant.CapitalRecordPerPage, begindate, enddate + " 23:59:59", 
                    "/financial/record.html?type=" + type + "&symbol=" + symbol + "&datetype=" + datetype + "&begindate="  //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
                            + begindate + "&enddate=" + enddate + "&");  //$NON-NLS-2$
            page = userCapitalService.listVirtualCapitalOperation(page, operation);
            jsonObject.put("list", page); 
        ...
        String select = ""; 
        if (type == 1) {
            select = I18NUtils.getString("financial.account.recordselectr"); 
        }
        if (type == 2) {
            select = I18NUtils.getString("financial.account.recordselectw"); 
        }
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="/financial/commission_json",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult commission(
        ...
        jsonObject1.put("symbol", 9); 
        jsonObject1.put("shortName", "GSET");  //$NON-NLS-2$
        jsonObject1.put("amount", userCapitalService.getCommissionTotalByID(fuser.getFid(), 9).toPlainString()); 
        jsonArray.add(0, jsonObject1);

        //BTC
        jsonObject1 = new JSONObject();
        jsonObject1.put("symbol", 1); 
        jsonObject1.put("shortName", "BTC");  //$NON-NLS-2$
        jsonObject1.put("amount", userCapitalService.getCommissionTotalByID(fuser.getFid(), 1).toPlainString()); 
        jsonArray.add(1, jsonObject1);

        //ETH
        jsonObject1 = new JSONObject();
        jsonObject1.put("symbol", 4); 
        jsonObject1.put("shortName", "ETH");  //$NON-NLS-2$
        jsonObject1.put("amount", userCapitalService.getCommissionTotalByID(fuser.getFid(), 4).toPlainString()); 
        ...
        jsonObject.put("introurl", "/user/intro.html?intro=" + fuser.getFid());  //$NON-NLS-2$
        return ReturnResult.SUCCESS(jsonObject);
    }
    ...
    @ResponseBody
    @ApiOperation("")
	@RequestMapping(value="/financial/transfer_record_json",method = {RequestMethod.GET,RequestMethod.POST})
    public ReturnResult transferRecord(
            @RequestParam(required = false, defaultValue = "0") Integer coinId,
            @RequestParam(required = false, defaultValue = "1") Integer currentPage,
            ...
            if(coinId != 0) {
            	coinType = redisHelper.getCoinType(coinId);
                if (coinType == null || coinType.getStatus().equals(SystemCoinStatusEnum.ABNORMAL.getCode())) {
                    return ReturnResult.FAILUER(""); 
                }
            }else {
            ...
            FUser fuser = getCurrentUserInfoByToken();
            Pagination<FVirtualCapitalOperationDTO> page = new Pagination<FVirtualCapitalOperationDTO>(currentPage,
            		pageSize, begindate, enddate,null);
            FVirtualCapitalOperationDTO operation = new FVirtualCapitalOperationDTO();
            operation.setFuid(fuser.getFid());
            if (type != 0) {
            	operation.setFtype(type);
			}
            //如果是从币就使用 fcoinid去查，如果是主币就用walletCoinId去查
            if(coinType != null) {
            	if(coinType.getIsSubCoin()) {
                	operation.setFcoinid(coinId);
                }else {
                	operation.setWalletCoinId(coinId);
                }
            }
            operation.setFcoinname(coinName);
            page = userCapitalService.listVirtualCapitalOperation(page, operation);
            for (FVirtualCapitalOperationDTO dto : page.getData()) {
            	if (dto.getFtype() == VirtualCapitalOperationTypeEnum.COIN_OUT.getCode()) {
            		dto.setFstatus_s(I18NUtils.getString("enum.capital.withdraw.status"+dto.getFstatus()));
				} else {
					dto.setFstatus_s(I18NUtils.getString("enum.capital.recharge.status"+dto.getFstatus()));
				}
			}
            if(page != null) {
            	JSONObject jsonObject = new JSONObject();
