...
@Controller
@ApiIgnore
public class VCCController extends JsonBaseController {
    ...
    @PassToken
    @ApiOperation("")
	@RequestMapping(value="/coin/vcc/recharge",method = {RequestMethod.GET,RequestMethod.POST})
    public String ethRecharge(
        ...
        if (coinType == null) {
            return ReturnMsg(10001, I18NUtils.getString("ETHController.6")); 
        }
        ...
        if (coinType.getShortName().equals("VCC")) { 
            baseCoinType = coinType;
        } else if (coinType.getCoinType().equals(SystemCoinSortEnum.VCC.getCode())) {
            baseCoinType = redisHelper.getCoinTypeShortName("VCC"); 
        }
        if (baseCoinType == null) {
            return ReturnMsg(10001, I18NUtils.getString("ETHController.9")); 
        }
        //验证签名
        try {
            ...
            CoinDriver coinDriver = new CoinDriverFactory.Builder(coinType.getCoinType(), coinType.getWalletLink(), coinType.getChainLink())
                    ...
                    .getDriver();
            if (coinDriver == null) {
                return ReturnMsg(10001, I18NUtils.getString("ETHController.10")); 
            }
            signVerify = coinDriver.getETCSHA3(signVerify);
            if (!signVerify.equals(sign)) {
                return ReturnMsg(10001, I18NUtils.getString("ETHController.11")); 
            }
        } catch (Exception e) {
            logger.error("ethRecharge sign error，symbol：{},e:{}", symbol,e); 
            return ReturnMsg(10001, I18NUtils.getString("ETHController.13")); 
        }
        ...
        try {
            ...
            logger.error("ethRecharge 服务器错误，symbol：{},e:{}", symbol,e); 
            return ReturnMsg(10001, I18NUtils.getString("ETHController.19")); 
        }
