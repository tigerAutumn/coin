...
import com.qkwl.web.utils.HttpHolderUtils;
...
public class FrontUserJsonController extends JsonBaseController {
	 ...
	 */
	@ApiOperation("")
	@RequestMapping(value="/user/oauth.html",method = {RequestMethod.GET,RequestMethod.POST})
	@ResponseBody
	...
	@PassToken
	@ApiOperation("")
	@RequestMapping(value="/user/check_user_exist",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult chcekregname(
		...
		if(flag) {
			if (type == 0 ) {
				return ReturnResult.FAILUER(I18NUtils.getString("user.security.error.1006")); 
			} else if (type == 1 ) {
				return ReturnResult.FAILUER(I18NUtils.getString("com.validate.error.11005")); 
			}else if (type == 2){
	...
	@PassToken
	@ApiOperation("")
	@RequestMapping(value="/user/send_reg_email",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult sendemail(
		...
		try {
			result = validateHelper.mailSendCode(address, PlatformEnum.BC, super.getLanEnum(), HttpRequestUtils.getIPAddress(),null);
		} catch (Exception e) {
	...
	@PassToken
	@ApiOperation("")
	@RequestMapping(value="/user/send_sms",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult sendphone(
		...
		if (!phone.equals("0")) { 
			if (!PhoneValiUtil.checkPhoneNumber(phone, area)) {
				return ReturnResult.FAILUER(I18NUtils.getString("com.validate.error.11008")); 
			}
			fuser.setFtelephone(phone);
			boolean flag = this.userService.selectIsExistByParam(fuser);
			if (flag) {
				return ReturnResult.FAILUER(I18NUtils.getString("com.validate.error.11003")); 
			}
			fuser.setFagentid(WebConstant.BCAgentId);
		} else {
			//缓存用户
	...
	@PassToken
	@ApiOperation("")
	@RequestMapping(value="/user/send_bank_sms",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult sendBindBankCode() throws Exception {
		FUser userInfo = getCurrentUserInfoByToken();
		if (userInfo == null){
			return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.38")); 
		}
		if (!userInfo.getFistelephonebind()){
			return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.39")); 
		}
		boolean result = false;
		try {
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/bind_google_device",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult googleAuth() throws Exception {
		...
		try {
			...
			logger.error("googleAuth err e:{}", e); 
		}
		...
		if(!StringUtils.isEmpty(systemArgs)) {
			wechatGoogleUrl = systemArgs.replace("#googleUrl#", fuser.getFgoogleurl()); 
		}
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/google_auth",method = {RequestMethod.GET,RequestMethod.POST})
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/get_google_key",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult getGoogleTotpKey(
		...
		if (!checkResult.getSuccess()) {
			return ReturnResult.FAILUER(I18NUtils.getString("com.validate.error." + checkResult.getCode(), checkResult.getData())); 
		}
		jsonObject.put("qecode", fuser.getFgoogleurl()); 
		jsonObject.put("code", 0); 
		jsonObject.put("totpKey", fuser.getFgoogleauthenticator()); 
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/modify_passwd",method = {RequestMethod.GET,RequestMethod.POST})
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/real_name_auth",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult userRealNameAuth(
		...
		if (StringUtils.isEmpty(realname) || identitytype == -1 || StringUtils.isEmpty(identityno)) {
			return ReturnResult.FAILUER(I18NUtils.getString("com.public.error.10005")); 
		}
		// 判断非空
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/save_withdraw_address",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult saveWithdrawBtcAddr(
			@RequestParam(required = false, defaultValue = "0") String phoneCode,
			@RequestParam(required = false, defaultValue = "0") String totpCode,
			...
			/*if(coinType.getCoinType().equals(SystemCoinSortEnum.GXS.getCode())
				...
				if(!withdarwheader.equals("0x")){
					return ReturnResult.FAILUER(ResourceBundleUtils.getMessage("com.validate.error.11017"));
				}
			}
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/del_withdraw_address",method = {RequestMethod.GET,RequestMethod.POST})
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/save_bankinfo",method = {RequestMethod.GET,RequestMethod.POST})
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/login_json",method = {RequestMethod.GET,RequestMethod.POST})
	...
	public ReturnResult authApi(@RequestParam(required = true,defaultValue = "0") String phone,
		...
		if ("0".equals(phone) || "0".equals(phoneCode) || "0".equals(areaCode)) {  //$NON-NLS-2$ //$NON-NLS-3$
			return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.92")); 
		}
		FUser fuser = getCurrentUserInfoByToken();
		if (null == fuser){
			return ReturnResult.FAILUER(ReturnResult.FAULURE_USER_NOT_LOGIN,I18NUtils.getString("FrontUserJsonController.93"));
		}
		//检查用户手机号
		if (!fuser.getFtelephone().equals(phone)){
			return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.94")); 
		}
			...
			if (fApiAuth == null){
				return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.95")); 
			}else{
				JSONObject jsonObject = new JSONObject();
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/send_api_sms",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult sendApiPhoneCode(
			@RequestParam(required = true, defaultValue = "0") String phone,
			@RequestParam(required = true, defaultValue = "0") String area) throws Exception {

		if ("0".equals(phone) || "0".equals(area)){  //$NON-NLS-2$
			return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.101")); 
		}
		...
		boolean flag = this.userService.selectIsExistByParam(fuser);
		if (!flag) {
			return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.102")); 
		}
	...
	@PassToken
	@ApiOperation("")
	@RequestMapping(value="/user/phonereg_json",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult regPhone() throws Exception{
		JSONObject jsonObject=new JSONObject();
		jsonObject.put("areaCodes", LuangeHelper.getAreaCode(LocaleEnum.ZH_CN.getCode())); 
		jsonObject.put("defaultAreaName", I18NUtils.getString("FrontUserJsonController.106"));  //$NON-NLS-2$
		jsonObject.put("defaultAreaCode", "86");  //$NON-NLS-2$
		return ReturnResult.SUCCESS(jsonObject);
	}
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/register_json",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult register(@RequestParam(required = false, defaultValue = "") String uid) throws Exception {
		// 推荐注册
		FUser intro = null;
			if (uid.isEmpty()) {
				Cookie cs[] = HttpHolderUtils.getHttpServletRequest().getCookies();
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/authlist",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult authApiList() throws Exception{
		FUser fuser = getCurrentUserInfoByToken();
		if (null == fuser){
			return ReturnResult.FAILUER(ReturnResult.FAULURE_USER_NOT_LOGIN,I18NUtils.getString("FrontUserJsonController.111"));
		}
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/security_json",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult userSecurity() throws Exception {
			...
			if (fuser.getFtelephone() != null && !fuser.getFtelephone().equals("")){ 
				...
				jsonObject.put("phoneString", "+"+fuser.getFareacode()+" "+phoneString);  //$NON-NLS-2$ //$NON-NLS-3$
			}
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/user_loginlog_json",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult userLoginlog(@RequestParam(required = false, defaultValue = "1") Integer currentPage) throws Exception {
		...
		for (FLogUserAction action : flogs.getData()) {
			action.setFcontent(I18NUtils.getString("UserLoginType."+action.getFdatatype()));
			action.setStatusString(I18NUtils.getString("user.action.success"));
		}
		JSONObject jsonObject=new JSONObject();
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/user_settinglog_json",method = {RequestMethod.GET,RequestMethod.POST})
	public ReturnResult userSettinglog(@RequestParam(required = false, defaultValue = "1") Integer currentPage) throws Exception {
		...
		for (FLogUserAction action : flogs.getData()) {
			action.setFtype_s(I18NUtils.getString("LogUserActionEnum."+action.getFtype()));
		}
		JSONObject jsonObject=new JSONObject();
		jsonObject.put("flogs", flogs); 
		return ReturnResult.SUCCESS(jsonObject);
	...
	@ResponseBody
	@ApiOperation("")
	@RequestMapping(value="/user/user_favorite_json",method = {RequestMethod.GET,RequestMethod.POST})
			...
			if(userInfo != null) {
				...
				if(operate == FavoriteOperateTypeEnum.GET.getCode() ) {
			        ...
			        if (tradeType == null || tradeType.getStatus().equals(SystemTradeStatusEnum.ABNORMAL.getCode())) {
			                return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.145")); 
			        }
				...
				return ReturnResult.FAILUER(I18NUtils.getString("FrontUserJsonController.146")); 
			}
