...
public class MQSend {
	...
	@Resource(name="c2cEntrustStatusProducer")
	private Producer c2cEntrustStatusProducer;
	 ...
	 *
	 */
	public void sendC2CEntrustStatus(int orderId) {
		String id = Integer.toString(orderId);
		Message message = new Message(MQTopic.c2cEntrustStatus, MQConstant.TAG_C2C_ENTRUST_STATUS, id.getBytes());
		logger.info("-----------------------topic:{}",MQTopic.c2cEntrustStatus);
		message.setKey("C2C_ENTRUST_STATUS_"+orderId);
		c2cEntrustStatusProducer.sendAsync(message, new SendCallback(){
			@Override
			public void onSuccess(SendResult sendResult) {}
			@Override
			public void onException(OnExceptionContext context) {
