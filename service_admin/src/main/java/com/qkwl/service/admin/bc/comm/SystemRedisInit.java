...
import com.qkwl.common.dto.Enum.BankStatusEnum;
...
import com.qkwl.service.admin.bc.dao.FAboutTypeMapper;
import com.qkwl.service.admin.bc.dao.FAgentMapper;
import com.qkwl.service.admin.bc.dao.FApiAuthMapper;
...
import com.qkwl.service.admin.elasticsearch.SystemTradeTypeSearchMapper;
...
public class SystemRedisInit implements IRedisService {
    ...
    @Autowired
    private TradeTypeRulesMapper tradeTypeRulesMapper;
    @Autowired
    MultiLangMapper multiLangMapper;
    @Autowired
    private SystemTradeTypeSearchMapper systemTradeTypeSearchMapper;
    ...
    public void redisInit() {
        try {
            ...
            System.out.println("SystemRedisInit --------------> otc支付类型初始化开始");
            initOtcPayment();

            System.out.println("SystemRedisInit --------------> otc法币类型初始化开始");
            initOtcCurrency();

            System.out.println("SystemRedisInit --------------> 交易对组初始化开始");
            initMatchGroup();

            System.out.println("SystemRedisInit --------------> 交易对规则初始化开始");
            initTradeRule();
            
            System.out.println("SystemRedisInit --------------> 多语言初始化开始");
            initMultiLang();

            System.out.println("SystemRedisInit --------------> 历史版本初始化开始");
            initHistoricVersion();
    ...
    public void clearRedis() {
        ...
        System.out.println("开始支付类型");
        deleteNoExpire(RedisConstant.PAYMENT_LIST_KEY);
        
        System.out.println("开始清空多语言redis");
        deleteNoExpire(RedisConstant.MULTI_LANG_KEY);
        
        System.out.println("开始清空历史版本redis");
        deleteNoExpire(RedisConstant.HISTORIC_VERSION);
    ...
    public void resetRedis(int type) {
        ...
        if(type == RedisTypeEnum.MULTI_LANG.getCode()){
          System.out.println("开始重置多语言redis");
          initMultiLang();
      }
        ...
        if (type == RedisTypeEnum.Coin.getCode()) {
            System.out.println("开始重置币种设置和币种信息和币种类型");
            initSystemCoinType();
            initCoinSetting();
            initCoinInfo();
        }

        if (type == RedisTypeEnum.Payment.getCode()) {
            System.out.println("开始重置otc支付类型");
            initOtcPayment();
        }

        if (type == RedisTypeEnum.Currency.getCode()) {
            System.out.println("开始重置otc法币类型");
            initOtcCurrency();
        }

        if (type == RedisTypeEnum.MatchGroup.getCode()) {
            System.out.println("开始重置交易对组");
            initMatchGroup();
        }

        if (type == RedisTypeEnum.TradeRule.getCode()) {
            System.out.println("开始重置交易对规则");
            initTradeRule();
        }
        if (type == RedisTypeEnum.HISTORIC_VERSION.getCode()) {
            System.out.println("开始重置历史版本");
            initHistoricVersion();
        }
    }
    ...
    public void clearRedis(int type) {
        ...
        if (type == RedisTypeEnum.Coin.getCode()) {
            ...
            deleteNoExpire(RedisConstant.SYS_VIRTUALCOINFEES_KEY);
            deleteNoExpire(RedisConstant.COIN_INFO_KEY);
            deleteNoExpire(RedisConstant.COIN_TYPE);
        }
        if (type == RedisTypeEnum.Trade.getCode()) {
            System.out.println("开始清空交易redis");
            deleteNoExpire(RedisConstant.TRADE_KEY);
        ...
        if (type == RedisTypeEnum.TradeRule.getCode()) {
            System.out.println("开始清空交易redis");
            deleteNoExpire(RedisConstant.TRADERULE);
        }
        if (type == RedisTypeEnum.MULTI_LANG.getCode()) {
            System.out.println("开始清空多语言redis");
            deleteNoExpire(RedisConstant.MULTI_LANG_KEY);
    ...
    public void initWebArticle(int type) {
        try {
            ...
            List<FSystemLan> selectAll = languageTypeMapper.selectAll();
            for (FSystemLan fSystemLan : selectAll) {
                for (FAgent fAgent : agents) {
                    ...
                    for (FArticleType fArticleType : articleTypes) {
                        ...
                        //页码        key+语言+客户端+券商+新闻类型
                        setNoExpire(RedisConstant.WEB_ARTICLECOUNT_KET + fSystemLan.getFid() + "_" + type + "_" + fAgent.getFid() + "_" + fArticleType.getFtypeid(), articlePageCountRedis);
                        ...
                        if (count > 0) {
                            ...
                            for (int i = 0; i < pageNum; i++) {
                                ...
                                setNoExpire(RedisConstant.WEB_ARTICLE_KET + fSystemLan.getFid() + "_" + type + "_" + fAgent.getFid() + "_" + fArticleType.getFtypeid() + "_" + (i + 1), articlesRedis);
                                for (FArticle fArticle : articleList) {
                                    RedisObject articleRedis = new RedisObject();
                                    articleRedis.setExtObject(fArticle);
                                    setNoExpire(RedisConstant.WEB_ARTICLEDETAIL_KET + fSystemLan.getFid() + "_" + type + "_" + fAgent.getFid() + "_" + fArticle.getFid(), articleRedis);
                                }
                            }
                        }
                    }
            ...
            // 主站数据         新增文章类型后的券商id为0，此部分代码与上面一致，这是券商id改成0去查询
            for (FSystemLan fSystemLan : selectAll) {
                List<FArticleType> articleTypes = articleTypeMapper.selectAll(fSystemLan.getFid());
                for (FArticleType fArticleType : articleTypes) {
                    Map<String, Object> map = new HashMap<String, Object>();
                    ...
                    RedisObject articlePageCountRedis = new RedisObject();
                    articlePageCountRedis.setExtObject(count);
                    setNoExpire(RedisConstant.WEB_ARTICLECOUNT_KET + fSystemLan.getFid() + "_" + type + "_" + 0 + "_" + fArticleType.getFtypeid(), articlePageCountRedis);
                    if (count > 0) {
                        int rest = count % 10;
                        int pageNum = 1;
                        if (rest > 0) {
                        ...
                        for (int i = 0; i < pageNum; i++) {
                            ...
                            RedisObject articlesRedis = new RedisObject();
                            articlesRedis.setExtObject(articleList);
                            setNoExpire(RedisConstant.WEB_ARTICLE_KET + fSystemLan.getFid() + "_" + type + "_" + 0 + "_" + fArticleType.getFtypeid() + "_" + (i + 1), articlesRedis);
                            for (FArticle fArticle : articleList) {
                                RedisObject articleRedis = new RedisObject();
                                articleRedis.setExtObject(fArticle);
                                //setNoExpire(RedisConstant.WEB_ARTICLEDETAIL_KET +fSystemLan.getFid() + "_" + type  + "_" + 0 + "_" + fArticle.getFid(), articleRedis);
                                setNoExpire(RedisConstant.WEB_ARTICLEDETAIL_KET + fArticle.getFid(), articleRedis);
                            }
                        }
                    }
                }
     ...
     * 初始化支付类型
     */
    public void initOtcPayment() {
        List<OtcPayment> selectAll = otcPaymentMapper.selectAll();
        RedisObject redisObject = new RedisObject();
        redisObject.setExtObject(selectAll);
        setNoExpire(RedisConstant.PAYMENT_LIST_KEY, redisObject);
    }

    /**
     * 初始化法币类型
     */
    public void initOtcCurrency() {
        List<OtcCurrency> selectAll = otcCurrencyMapper.selectAll();
        RedisObject redisObject = new RedisObject();
        redisObject.setExtObject(selectAll);
        setNoExpire(RedisConstant.CURRENCY_LIST_KEY, redisObject);
    }

    /**
     * 初始化交易对组
     */
    public void initMatchGroup() {
        List<MatchGroup> matchGroupList = matchGroupMapper.selectAll();
        ...
        for (int i = 0; i < matchGroupList.size(); i++) {
            ...
            value.append("\"").append(matchGroupList.get(i).getGroupName()).append("\":\"")
                    .append(matchGroupList.get(i).getTradeIds()).append("\"");
        }
        value.append("}");
        memCache.setNoExpire(RedisConstant.MATCH_GROUP, value.toString());
    }

    /**
     * 初始化交易对规则
     */
    public void initTradeRule() {
        List<SystemTradeTypeRule> tradeRuleList = tradeTypeRulesMapper.selectAll();
        for (SystemTradeTypeRule tradeRule : tradeRuleList) {
            String tradeRuleStr = memCache.get(RedisConstant.TRADERULE + tradeRule.getTradeid() + "_" + tradeRule.getRectype());
            RedisObject redisObject = new RedisObject();
            List<SystemTradeTypeRule> list = new ArrayList<>();
            if (!StringUtils.isEmpty(tradeRuleStr)) {
                JSONObject obj = JSON.parseObject(tradeRuleStr);
                list = JSONArray.parseArray(obj.getJSONArray("extObject").toString(), SystemTradeTypeRule.class);
            }
            list.add(tradeRule);
            redisObject.setExtObject(list);
            setNoExpire(RedisConstant.TRADERULE + tradeRule.getTradeid() + "_" + tradeRule.getRectype(), redisObject);
        }
    }
     ...
     * 初始化交易对规则
     */
    public void initHistoricVersion() {
        List<HistoricVersion> versionList = historicVersionMapper.selectLimitTen();
        RedisObject redisObject = new RedisObject();
        redisObject.setExtObject(versionList);
        setNoExpire(RedisConstant.HISTORIC_VERSION, redisObject);
    }
    ...
    /**
     * 初始化初始化币种列表(多平台)及币种归类
     */
    public void initSystemCoinType() {
        //deleteNoExpire(RedisConstant.COIN_KEY);
        //deleteNoExpire(RedisConstant.COIN_TYPE);
        //币种归类map
		/*Map<String,Integer> coinTypeMap = new HashMap<>();
		for(SystemCoinSortEnum s:SystemCoinSortEnum.values()) {
			coinTypeMap.put(s.getShortName(), s.getCode());
		}*/
        ...
        for (PlatformEnum platformEnum : PlatformEnum.values()) {
            ...
            for (SystemCoinType coin : coinTypeList) {
				...
				/*Integer integer = coinTypeMap.get(coin.getShortName());
				if(integer != null) {
					setNoExpire(RedisConstant.COIN_TYPE + integer, coins);
				}*/
            }
    ...
    public void initSystemTradeType() {
        List<FAgent> agents = agentMapper.selectAll();
        List<SystemTradeType> tradeTypeList = systemTradeTypeMapper.selectAll();//币币交易类型
        // 扩展数据加载
        ...
        _logger.info(">>>>>>开始清空索引systemTradeTypeIndex");
        systemTradeTypeSearchMapper.deleteAll();
        _logger.info(">>>>>>将交易币对索引到elasticsearch");
        Iterable<SystemTradeType> savedData = systemTradeTypeSearchMapper.saveAll(agentsTradeTypeList);
        Iterator<SystemTradeType> iterator = savedData.iterator();
        while(iterator.hasNext()) {
            _logger.info("<<<<<<<<索引的数据:{}",JSON.toJSONString(iterator.next()));
        }
        }, executor);
     ...
     * @param key key
     */
    public Object getSystemArgs(String key) {
        String str = memCache.get(RedisConstant.ARGS_KET + key);
        if (!StringUtils.isEmpty(str)) {
            RedisObject redisObject = JSON.parseObject(str, RedisObject.class);
            return redisObject.getExtObject();
        }
        return null;
    }
     ...
     * 初始化app版本信息
     */
    public void initAppVersionInfo() {
        VersionUpgradeInfo android = new VersionUpgradeInfo();
        android.setUpdateVersion((String) getSystemArgs("AndroidVersion"));
        Integer AndroidIsUpdate = Integer.valueOf((String) getSystemArgs("AndroidIsUpdate"));
//		if(AndroidIsUpdate == 1) {
//			android.setUpdateType(3);
//		}else {
//			android.setUpdateType(2);
//		}
        android.setUpdateType(AndroidIsUpdate);
        android.setDownloadUrl((String) getSystemArgs("AndroidDownloadUrl"));
        android.setUpdateContent((String) getSystemArgs("AndroidUpdateContent"));
        android.setUpdateVersionNumber((String) getSystemArgs("updateVersionNumber"));
        RedisObject redisObj = new RedisObject();
        redisObj.setExtObject(android);
        setNoExpire(RedisConstant.APP_VERSION_KEY + 1, redisObj);

        VersionUpgradeInfo ios = new VersionUpgradeInfo();
        ios.setUpdateVersion((String) getSystemArgs("IosVersion"));
        Integer iosIsUpdate = Integer.valueOf((String) getSystemArgs("IosIsUpdate"));
        ios.setUpdateType(iosIsUpdate);
        ios.setDownloadUrl((String) getSystemArgs("IosDownloadUrl"));
        ios.setUpdateContent((String) getSystemArgs("IosUpdateContent"));
        RedisObject redisObjios = new RedisObject();
        redisObjios.setExtObject(ios);
        setNoExpire(RedisConstant.APP_VERSION_KEY + 2, redisObjios);

        VersionUpgradeInfo web = new VersionUpgradeInfo();
        web.setUpdateVersion((String) getSystemArgs("WebVersion"));
        Integer webIsUpdate = Integer.valueOf((String) getSystemArgs("WebIsUpdate"));
        web.setUpdateType(webIsUpdate);
        web.setDownloadUrl((String) getSystemArgs("WebDownloadUrl"));
        web.setUpdateContent((String) getSystemArgs("WebUpdateContent"));
        RedisObject redisObjweb = new RedisObject();
        redisObjweb.setExtObject(web);
        setNoExpire(RedisConstant.APP_VERSION_KEY + 3, redisObjweb);
    }
  ...
  public void initMultiLang() {
     List<MultiLang> list = multiLangMapper.selectByExample(null);
    for (MultiLang multiLang : list) {
        RedisObject obj = new RedisObject();
        obj.setExtObject(multiLang);
        setNoExpire(RedisConstant.MULTI_LANG_KEY + multiLang.getCode() + "_" + multiLang.getLang(), obj);
    }
  }
}
