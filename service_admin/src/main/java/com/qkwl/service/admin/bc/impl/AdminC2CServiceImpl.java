...
public class AdminC2CServiceImpl implements IAdminC2CService {
	...
	public PageInfo<UserC2CEntrust> selectUserC2CEntrustList(UserC2CEntrust u,Integer pageNum,Integer pageSize) {
		try {
			...
			for (UserC2CEntrust userC2CEntrust : list) {
				userC2CEntrust.setStatusString(UserC2CEntrustStatusEnum.getValueByCode(userC2CEntrust.getStatus()));
			}
	...
	public ReturnResult auditPassThrough(Integer id,Integer adminId,Integer type) {
		try {
			...
			if(entrust.getType() != null && entrust.getType() == UserC2CEntrustTypeEnum.recharge.getCode()) {
				...
				if(!walletUtils.change(entrust.getUserId(), entrust.getCoinId(), WalletUtils.total_add, entrust.getAmount())) {
					return ReturnResult.FAILUER("更改用户钱包失败");
				}
				...
				wallet.setGmtModified(date);*/
				if(!walletUtils.change(entrust.getUserId(), entrust.getCoinId(), WalletUtils.frozen_sub, entrust.getAmount())) {
					return ReturnResult.FAILUER("更改用户钱包失败");
				}
			}else {
			...
			// MQ
			FUser user = userMapper.selectByPrimaryKey(entrust.getUserId());
			FUserExtend userExtend = userExtendMapper.selectByUid(entrust.getUserId());
			Integer langCode = 1;
			if (userExtend != null) {
				langCode = LocaleEnum.getCodeByName(userExtend.getLanguage());
			}
			SystemCoinType coinType = systemCoinTypeMapper.selectByPrimaryKey(entrust.getCoinId());
			if(entrust.getType() != null && entrust.getType() == UserC2CEntrustTypeEnum.recharge.getCode()) {
				...
				if (user.getFistelephonebind()) {
					validateHelper.smsCoinToAccount(user.getFareacode(), user.getFtelephone(), PlatformEnum.BC.getCode(),
							BusinessTypeEnum.SMS_C2C_BUY.getCode(), entrust.getAmount(), coinType.getShortName(), langCode);
				}
			}else {
				...
				if (user.getFistelephonebind()) {
					validateHelper.smsCoinToAccount(user.getFareacode(), user.getFtelephone(), PlatformEnum.BC.getCode(),
							BusinessTypeEnum.SMS_C2C_SELL.getCode(), entrust.getAmount(), coinType.getShortName(), langCode);
				}
			}
	...
	public ReturnResult auditReject(Integer id,Integer adminId,Integer type) {
		try {
			...
			if(entrust.getType() != null && entrust.getType() == UserC2CEntrustTypeEnum.withdraw.getCode()) {
				...
				if(!walletUtils.change(entrust.getUserId(), entrust.getCoinId(), WalletUtils.frozen_to_total, entrust.getAmount())) {
					return ReturnResult.FAILUER("更改用户钱包失败");
				}
	...
	public int saveBankInfo(FSystemBankinfoWithdraw fSystemBankinfoWithdraw ,Integer adminId) {
		try {
			...
			//重置redis
			systemRedisInit.initBankinfoWithdraw();
			return insert;
		} catch (Exception e) {
