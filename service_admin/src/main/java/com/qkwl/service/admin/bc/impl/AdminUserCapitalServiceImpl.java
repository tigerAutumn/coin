...
import com.qkwl.service.admin.bc.dao.AirdropRecordMapper;
...
//			}
	...
	public Pagination<FVirtualCapitalOperationDTO> selectVirtualCapitalOperationList(
		...
		if(count > 0) {
			...
			for (FVirtualCapitalOperationDTO dto : list) {
				if (dto.getFtype() == VirtualCapitalOperationTypeEnum.COIN_IN.getCode()) {
					dto.setFstatus_s(VirtualCapitalOperationInStatusEnum.getEnumString(dto.getFstatus()));
					String name = "";
					if (dto.getFstatus() == VirtualCapitalOperationInStatusEnum.WAIT_0) {
						name = "0/项确认"; 
					} else if (dto.getFstatus() == VirtualCapitalOperationInStatusEnum.WAIT_1) {
						name = "1/项确认"; 
					} else if (dto.getFstatus() == VirtualCapitalOperationInStatusEnum.WAIT_2) {
						name = "2/项确认";
					} else if (dto.getFstatus() == VirtualCapitalOperationInStatusEnum.SUCCESS) {
						name = "充值成功";
					}
					dto.setFstatus_s(name);
				} else if (dto.getFtype() == VirtualCapitalOperationTypeEnum.COIN_OUT.getCode()) {
					String name = "";
					if (dto.getFstatus() == VirtualCapitalOperationOutStatusEnum.WaitForOperation) {
						name = "等待提现";
					} else if (dto.getFstatus() == VirtualCapitalOperationOutStatusEnum.OperationLock) {
						name = "正在处理";
					} else if (dto.getFstatus() == VirtualCapitalOperationOutStatusEnum.OperationSuccess) {
						name = "提现成功";
					} else if (dto.getFstatus() == VirtualCapitalOperationOutStatusEnum.Cancel) {
						name = "用户撤销";
					} else if (dto.getFstatus() == VirtualCapitalOperationOutStatusEnum.LockOrder) {
						name = "锁定";
					}
					dto.setFstatus_s(name);
				}
	...
	public Map<Integer,UserCoinWallet> selectUserVirtualWallet(int fuid) {
		try {
			...
			return selectByUid.parallelStream().collect(Collectors.toMap(UserCoinWallet::getCoinId, f ->f));
		} catch (Exception e) {
	...
	public boolean platformExchange(Integer recordId, Integer adminId ,SystemCoinType systemCoinType) throws BCException {
		...
		if(!systemCoinType.getCoinType().equals(SystemCoinSortEnum.EOS.getCode()) 
				&& !systemCoinType.getCoinType().equals(SystemCoinSortEnum.BTS.getCode())
				&& !systemCoinType.getCoinType().equals(SystemCoinSortEnum.XRP.getCode())){
			...
			uidIn = userVirtualAddress.getFuid();
			if(!walletUtils.change(uidIn, record.getWalletCoinId(), walletUtils.total_add,  record.getFamount())) {
				throw new BCException("平台互转钱包转入失败");
			}
			tovirtualcaptualoperation.setFuid(userVirtualAddress.getFuid());
		}else {
			try {
				uidIn = Integer.valueOf(record.getMemo());
				if(!walletUtils.change(uidIn, record.getWalletCoinId(), walletUtils.total_add,  record.getFamount())) {
					throw new BCException("平台互转钱包转入失败");
				}
				tovirtualcaptualoperation.setFuid(uidIn);
			} catch (Exception e) {
				logger.error("用户memeo填写错误");
				throw new BCException("mome填写错误");
			}
			
		}
		// 添加一条转入地址的充值记录
		String uuid = "[平台互转]" + Utils.UUID() + "(" + record.getFuid() +")" ;
		...
		tovirtualcaptualoperation.setFuniquenumber(uuid);
		...
		tovirtualcaptualoperation.setWalletCoinId(record.getWalletCoinId());
		...
		//改记录，发mq
		updateRecordAndWallet(record);
		//平台互转给转入的用户发送充值消息
		platformExchangeRechargeMq(record, uidIn);
		logger.info("转账结束");
		return true;
	}
	...
	@Transactional(value="xaTransMgr", isolation = Isolation.REPEATABLE_READ, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	//@Transactional(isolation = Isolation.REPEATABLE_READ, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public boolean virtualCapitalWithdraw(Integer recordId, Integer adminId, CoinDriver coinDriver) throws BCException {
		FVirtualCapitalOperationDTO record =null ;
		try {
			record = virtualCapitalOperationMapper.selectAllById(recordId);
		} catch (Exception e) {
		...
		try {
			if (coinDriver.getCoinSort().equals(SystemCoinSortEnum.BTC.getCode())
					|| coinDriver.getCoinSort().equals(SystemCoinSortEnum.CHAIN33.getCode())
					|| coinDriver.getCoinSort().equals(SystemCoinSortEnum.RUBY.getCode())
					|| coinDriver.getCoinSort().equals(SystemCoinSortEnum.TCP.getCode())) {
				logger.info("开始转账btc,recordId:"+recordId);
				resultTX = snedBTC(coinDriver, record);
			}else if (coinDriver.getCoinSort().equals(SystemCoinSortEnum.ETH.getCode()) ) {
			...
			}else if(coinDriver.getCoinSort().equals(SystemCoinSortEnum.VCC.getCode()) ) {
				logger.info("开始转账vcc,recordId:"+recordId);
				resultTX = sendVCC(coinDriver, record);
			}else if(coinDriver.getCoinSort().equals(SystemCoinSortEnum.ETC.getCode()) ) {
				logger.info("开始转账ETC,recordId:"+recordId);
				resultTX = snedETC(coinDriver, record);
			}else if(coinDriver.getCoinSort().equals(SystemCoinSortEnum.EOS.getCode())) {
				logger.info("开始转账EOS,recordId:"+recordId);
				resultTX = snedEOS(coinDriver, record);
			} else if (coinDriver.getCoinSort().equals(SystemCoinSortEnum.BTS.getCode())) {
				logger.info("开始转账bts,recordId:"+recordId);
	...
	@Transactional(value="xaTransMgr", isolation = Isolation.REPEATABLE_READ, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	//@Transactional(isolation = Isolation.REPEATABLE_READ, propagation = Propagation.REQUIRED, rollbackFor = Exception.class)
	public boolean virtualCapitalWithdraw(Integer recordId, Integer adminId, SystemCoinType systemCoinType) throws BCException {
		FVirtualCapitalOperationDTO record =null ;
		try {
			record = virtualCapitalOperationMapper.selectAllById(recordId);
		} catch (Exception e) {
		...
		Integer nonceTmp = null;
		try {
			resultTX = CoinRpcUtlis.sendToAddress(systemCoinType, record);
		} catch (Exception e) {
			if(e instanceof BCException) {
				throw (BCException)e;
			}
			logger.info("转账异常 ,recordId:"+recordId,e);
			throw new BCException("打币链接钱包出现错误"+e.getMessage());
		}
		if (StringUtils.isEmpty(resultTX)) {
			logger.error("钱包连接错误  : " + resultTX + "_" + nonceTmp);
			throw new BCException("钱包连接错误  : " + resultTX + "_" + nonceTmp);
	...
	private void updateRecordAndWallet(FVirtualCapitalOperationDTO record) throws BCException{
		// 更新订单
		if (virtualCapitalOperationMapper.updateByPrimaryKey(record) <= 0) {
			logger.error("更新订单失败,recordId:"+record.getFid());
			throw new BCException("更新订单失败");
		}
		BigDecimal amount = MathUtils.add(MathUtils.add(record.getFamount(),
				record.getFfees()),record.getFbtcfees());
		//用户钱包扣钱
		if(!walletUtils.change(record.getFuid(), record.getWalletCoinId(), WalletUtils.frozen_sub, amount)) {
		...
		try {
			...
			FUser user = userMapper.selectByPrimaryKey(record.getFuid());
			FUserExtend userExtend = userExtendMapper.selectByUid(record.getFuid());
			Integer langCode = 1;
			if (userExtend != null) {
				langCode = LocaleEnum.getCodeByName(userExtend.getLanguage());
			}
			// 发送短信
			validateHelper.smsCoinToAccount(user.getFareacode(), user.getFtelephone(), record.getFplatform(),
					BusinessTypeEnum.SMS_WITHDRAW_TO_ACCOUNT.getCode(), record.getFamount(), record.getShortName(), langCode);
		} catch (Exception e) {
			logger.info("MQ 发送失败,record:"+record.getFid());
			e.printStackTrace();
		}
	}
	
	private void platformExchangeRechargeMq(FVirtualCapitalOperationDTO record , Integer uid) {
		// 平台互转发送充币信息
		if (uid != 0) {
			try {
				FUser fUser = userMapper.selectByPrimaryKey(uid);
				FUserExtend fUserExtend = userExtendMapper.selectByUid(uid);
				Integer code = 1;
				if (fUserExtend != null) {
					code = LocaleEnum.getCodeByName(fUserExtend.getLanguage());
				}
				// 发送短信
				validateHelper.smsCoinToAccount(fUser.getFareacode(), fUser.getFtelephone(), record.getFplatform(),
						BusinessTypeEnum.SMS_CHARGE_TO_ACCOUNT.getCode(), record.getFamount(), record.getShortName(), code);
			} catch (Exception e) {
	...
	public boolean updateVirtualCapital(Integer recordId, Integer adminId, BigDecimal amount, int addressNum,
		...
		if (addressNum > 0) {
			...
			if(!coinDriver.getCoinSort().equals(SystemCoinSortEnum.EOS.getCode()) 
					&& !coinDriver.getCoinSort().equals(SystemCoinSortEnum.BTS.getCode())
					&& !coinDriver.getCoinSort().equals(SystemCoinSortEnum.XRP.getCode())){
				List<FUserVirtualAddressDTO> userVirtualAddresses = userVirtualAddressMapper.getUserByAddress(address);
				if (userVirtualAddresses == null || userVirtualAddresses.size() != 1) {
					throw new BCException("平台互转转入地址为空");
				}
				// 转入地址信息
				FUserVirtualAddressDTO userVirtualAddress = userVirtualAddresses.get(0);
				uid = userVirtualAddress.getFuid();
				if(!walletUtils.change(userVirtualAddress.getFuid(), record.getWalletCoinId(), walletUtils.total_add,  record.getFamount())) {
					throw new BCException("平台互转钱包转入失败");
				}
				tovirtualcaptualoperation.setFuid(userVirtualAddress.getFuid());
			}else {
				
				if(!walletUtils.change(Integer.valueOf(record.getMemo()), record.getWalletCoinId(), walletUtils.total_add,  record.getFamount())) {
					throw new BCException("平台互转钱包转入失败");
				}
		...
		if(!walletUtils.change(fuid, record.getWalletCoinId(), WalletUtils.frozen_sub, amount)) {
			throw new BCException("更新用户钱包失败");
		}
		logger.info("转账结束");
		try {
			...
			FUserExtend userExtend = userExtendMapper.selectByUid(fuid);
			Integer langCode = 1;
			if (userExtend != null) {
				langCode = LocaleEnum.getCodeByName(userExtend.getLanguage());
			}
	...
	private String snedBTC(CoinDriver coinDriver,FVirtualCapitalOperationDTO record) throws BCException {
		ReturnResult sendToAddress = coinDriver.sendToAddress(record.getFwithdrawaddress(), record.getFamount(),record.getFbtcfees(), null, null, null, record.getFid().toString());
		if(sendToAddress.getCode() != 200) {
			throw new BCException(sendToAddress.getCode(),sendToAddress.getMsg());
		}
		return (String) sendToAddress.getData();
	}
	
	private String snedEOS(CoinDriver coinDriver,FVirtualCapitalOperationDTO record) throws BCException {
		ReturnResult sendToAddress = coinDriver.sendToAddress(record.getFwithdrawaddress(), record.getFamount(),null, null, null, null, record.getMemo());
		if(sendToAddress.getCode() != 200) {
			throw new BCException(sendToAddress.getCode(),sendToAddress.getMsg());
		}
		return (String) sendToAddress.getData();
	...
	private String snedETC(CoinDriver coinDriver,FVirtualCapitalOperationDTO record) throws BCException {
		String nonce = null;
		Integer nonceTmp;
		Long incrByKey = redisHelper.getIncrByKey(RedisConstant.ETC_CURRENT_NONCE);
		if(incrByKey != null && incrByKey != 0 && incrByKey != 1) {
			nonceTmp = incrByKey.intValue();
		}else{
			nonceTmp = coinDriver.getTransactionCount();
			redisHelper.setNoExpire(RedisConstant.ETC_CURRENT_NONCE, nonceTmp.toString());
		}
		nonce = "0x" + Integer.toHexString(nonceTmp);
		/*Integer nonceTmp = coinDriver.getTransactionCount();
		String nonce = "0x" + Integer.toHexString(nonceTmp);*/
		ReturnResult sendToAddress = coinDriver.sendToAddress(record.getFwithdrawaddress(), record.getFamount(),null, nonce,null,null,null);
		if(sendToAddress.getCode() == 403) {
			//如果密码错误，回滚一下nonce,再将异常抛出，回滚事务
			redisHelper.getDecrByKey(RedisConstant.ETC_CURRENT_NONCE);
			throw new BCException(sendToAddress.getCode(),sendToAddress.getMsg());
		}else if(sendToAddress.getCode() == 409) {
			//nonce too low ，删除nonce
			redisHelper.delete(RedisConstant.ETC_CURRENT_NONCE);
			throw new BCException(sendToAddress.getCode(),sendToAddress.getMsg());
		}else if(sendToAddress.getCode() == 500) {
			//未知错误
			throw new BCException(sendToAddress.getCode(),sendToAddress.getMsg());
		}else{
			return (String) sendToAddress.getData();
	...
	private String sendETH(CoinDriver coinDriver,FVirtualCapitalOperationDTO record) throws BCException {
		...
		nonce = "0x" + Integer.toHexString(nonceTmp);
		ReturnResult sendToAddress = coinDriver.sendToAddress(record.getFwithdrawaddress(), record.getFamount(),null, nonce,gasPrice,gas,null);
		if(sendToAddress.getCode() == 403) {
			//如果密码错误，回滚一下nonce,再将异常抛出，回滚事务
	...
	private String sendVCC(CoinDriver coinDriver,FVirtualCapitalOperationDTO record) throws BCException {
		//获取合适gwei
		BigDecimal estimatesmartfee = coinDriver.estimatesmartfee(0);
		String gasPrice = null;
		...
		String nonce = null;
		Integer nonceTmp;
		Long incrByKey = redisHelper.getIncrByKey(RedisConstant.VCC_CURRENT_NONCE);
		if(incrByKey != null && incrByKey != 0 && incrByKey != 1) {
			nonceTmp = incrByKey.intValue();
		}else{
			nonceTmp = coinDriver.getTransactionCount();
			redisHelper.setNoExpire(RedisConstant.VCC_CURRENT_NONCE, nonceTmp.toString());
		}
		nonce = "0x" + Integer.toHexString(nonceTmp);
		ReturnResult sendToAddress = coinDriver.sendToAddress(record.getFwithdrawaddress(), record.getFamount(),null, nonce,gasPrice,gas,null);
		if(sendToAddress.getCode() == 403) {
			//如果密码错误，回滚一下nonce,再将异常抛出，回滚事务
			redisHelper.getDecrByKey(RedisConstant.VCC_CURRENT_NONCE);
			throw new BCException(sendToAddress.getCode(),sendToAddress.getMsg());
		}else if(sendToAddress.getCode() == 409) {
			//nonce too low ，删除nonce
	...
	private String sendFOD(CoinDriver coinDriver,FVirtualCapitalOperationDTO record) throws BCException {
		...
		nonce = "0x" + Integer.toHexString(nonceTmp);
		ReturnResult sendToAddress = coinDriver.sendToAddress(record.getFwithdrawaddress(), record.getFamount(),null, nonce,gasPrice,null,null);
		if(sendToAddress.getCode() == 403) {
			//如果密码错误，回滚一下nonce,再将异常抛出，回滚事务
	...
	public boolean updateVirtualCapitalAccelerate(Integer recordId, Integer adminId, BigDecimal amount, String gasPrice,
		...
		try {
			if (coinDriver.getCoinSort().equals(SystemCoinSortEnum.ETH.getCode()) ) {
				...
				ReturnResult sendToAddress = coinDriver.sendToAddress(address, record.getFamount(),null, nonce,gasPrice,gas,null);
				if(sendToAddress.getCode() == 403) {
	...
	public boolean updateVirtualCapitalSuccessful(Integer recordId, Integer adminId, BigDecimal amount, String txhash) throws BCException {
		try {
			FVirtualCapitalOperationDTO record = virtualCapitalOperationMapper
					.selectAllById(recordId);
			if (!record.getFstatus().equals(VirtualCapitalOperationOutStatusEnum.LockOrder)) {
				throw new BCException("订单锁定中，不允许重复操作！");
			}
			int fuid = record.getFuid();
			int fcoinid = record.getFcoinid();
			if (!walletUtils
					.change(fuid, record.getWalletCoinId(), walletUtils.frozen_sub, amount)) {
				throw new BCException("钱包更改失败");
			}
		...
		/*if (userCoinWalletMapper.updateByPrimaryKey(userVirtualWallet) <= 0) {
			...
			logger.info("转账结束");
			// MQ
			mqSend.SendUserAction(record.getFagentid(), record.getFuid(),
					LogUserActionEnum.COIN_WITHDRAW,
					record.getFcoinid(), amount, record.getFfees(), record.getFbtcfees());

			// MQ
			mqSend.SendAdminAction(record.getFagentid(), record.getFadminid(), record.getFuid(),
					LogAdminActionEnum.SYSTEM_COIN_WITHDRAW_MANUAL_COMPLETION, record.getFcoinid(),
					record.getFamount());
			return true;
		}catch (Exception ex){
			logger.error("ex:{}",ex);
	...
	public boolean updateVirtualCapital(FAdmin fAdmin, FVirtualCapitalOperationDTO record) throws BCException {
		if (record.getFstatus() == VirtualCapitalOperationOutStatusEnum.Cancel) {
			...
			if(!walletUtils.change(record.getFuid(), record.getWalletCoinId(), WalletUtils.frozen_to_total, amount)) {
	...
	public boolean updateConsoleVirtualRecharge(FLogConsoleVirtualRecharge record) throws BCException {
		if (record.getFstatus() == OperationlogEnum.FFROZEN) {
			...
			UserCoinWallet userVirtualWallet = userCoinWalletMapper.selectByUidAndCoin(fuid, fcoinId);
			...
			int updateByPrimaryKey = userCoinWalletMapper.updateByPrimaryKey(userVirtualWallet);
			if(updateByPrimaryKey <= 0) {
				throw new BCException("更新用户钱包错误");
			}
			/*if(!walletUtils.change(fuid, fcoinId, WalletUtils.frozen_add, qty)) {
				throw new BCException("更新用户钱包错误");
			}*/
		}
		if (record.getFstatus() == OperationlogEnum.AUDIT) {
			...
			/*if(!walletUtils.change(fuid, fcoinId, WalletUtils.frozen_to_total, qty)) {
				throw new BCException("更新用户钱包错误");
			}*/
		}
	...
	@Override
	public Pagination<UserCoinWallet> importUserVirtualWalletListByCoin(Pagination<UserCoinWallet> pageParam, UserCoinWallet filterParam) {
		try {
		// 组装查询条件数据
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("offset", pageParam.getOffset());
		...
		map.put("coinId", filterParam.getCoinId());
		// 查询总数
		int count = userCoinWalletMapper.countImportAdminPage(map);
		if(count > 0) {
			// 查询数据
			List<UserCoinWallet> list = userCoinWalletMapper.getImportAdminPageList(map);
			// 设置返回数据
			pageParam.setData(list);
		}
		pageParam.setTotalRows(count);
	 ...
	 */
	@Override
	public BigDecimal selectVirtualWalletTotalAmount(Integer fuid, Integer coinid, Integer type, Integer status, Date start, Date end,Boolean isFrozen){
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("fuid", fuid);
			...
			logger.error("查询虚拟币充提记录异常",e);
		}
		return null;
	}
	
	@Override
	public Map<Integer,BigDecimal> selectVirtualWalletTotalAmountMap(Integer fuid, Integer type, Integer status, Date start, Date end,Date date,Boolean isFrozen){
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("fuid", fuid);
			map.put("type", type);
			...
			Map<Integer,BigDecimal> collect = totalAmountByTypeList.stream().collect(Collectors.toMap(FVirtualCapitalOperationDTO::getWalletCoinId, FVirtualCapitalOperationDTO::getFamount));
			return collect;
		} catch (Exception e) {
	...
	public boolean updateUserWallet(Integer uid, Integer coinId, BigDecimal amount){
		...
		try {
			return walletUtils.change(uid, coinId, WalletUtils.frozen_to_total, amount);
		} catch (Exception e) {
			logger.error("修改资金信息异常",e);
		}
		return false;
	}
	...
	public Result insertRecharge(FVirtualCapitalOperationDTO operation){
		...
		//如果是冻结的就查询是否白名单
		if(operation.getIsFrozen()) {
			Map<String,Object> param = new HashMap<>();
			param.put("userId", operation.getFuid());
			param.put("coinId", operation.getFcoinid());
			param.put("type", UserWhiteListTypeEnum.RECHARGE_OF_INNOVATION_ZONE.getCode());
	...
	public Result recheckVirtualRecharge(FVirtualCapitalOperationDTO operation) throws Exception{
		try {
			...
			SystemCoinType virtualCoinType = redisHelper.getCoinTypeSystem(virtualOperation.getFcoinid());
			...
			if(virtualOperation.getIsFrozen()) {
				if(!walletUtils.change(virtualOperation.getFuid(), virtualOperation.getWalletCoinId(), WalletUtils.depositFrozen_add, virtualOperation.getFamount())) {
					return Result.failure("更新用户钱包失败");
				}
				//修改用户创新区锁仓记录
				OrepoolRecord record = new OrepoolRecord();
				record.setUserId(virtualOperation.getFuid());
				record.setLockCoinId(virtualOperation.getFcoinid());
				...
				if(!walletUtils.change(virtualOperation.getFuid(), virtualOperation.getWalletCoinId(), WalletUtils.total_add, virtualOperation.getFamount())) {
					return Result.failure("更新用户钱包失败");
				}
			}
			if(virtualCapitalOperationMapper.updateByPrimaryKey(virtualOperation) <= 0){
				throw new BCException("更新充值记录失败！");
			}
			
			FUser user = userMapper.selectByPrimaryKey(virtualOperation.getFuid());
			FUserExtend userExtend = userExtendMapper.selectByUid(virtualOperation.getFuid());
			Integer langCode = 1;
			if (userExtend != null) {
				langCode = LocaleEnum.getCodeByName(userExtend.getLanguage());
			}
			...
			if (user.getFistelephonebind()) {
				validateHelper.smsCoinToAccount(user.getFareacode(), user.getFtelephone(), PlatformEnum.BC.getCode(),
						BusinessTypeEnum.SMS_CHARGE_TO_ACCOUNT.getCode(), virtualOperation.getFamount(),
						virtualOperation.getShortName(), langCode);
			}
	...
	@Override
	public PageInfo<DepositFrozenImbalance> selectdepositFrozenImbalancePage(DepositFrozenImbalance params,Integer page, Integer pageSize) {
		try {
			PageHelper.startPage(page, pageSize);
			List<DepositFrozenImbalance> selectByParam = depositFrozenImbalanceMapper.selectByParam(params);
			PageInfo<DepositFrozenImbalance> pageInfo = new PageInfo<DepositFrozenImbalance>(selectByParam);
			return pageInfo;
		} catch (Exception e) {
			logger.error("查询冻结不平衡列表异常",e);
		}
		return null;
	}
	...
	@Override
	public Map<Integer, BigDecimal> sumAirdropRecord(Integer uid) {
		try {
			List<AirdropRecord> selectSumByUid = airdropRecordMapper.selectSumByUid(uid);
			return selectSumByUid.stream().collect(Collectors.toMap(AirdropRecord::getCoinId,AirdropRecord::getAmount));
		} catch (Exception e) {
			logger.error("统计空投异常",e);
		}
		return null;
	}

	@Override
	public Map<Integer, OtcUserTransfer> sumOtcTransfer(Integer uid) {
		try {
			Map<Integer, OtcUserTransfer> map = new HashMap<>();
			List<Integer> typeList = new ArrayList<>();
			typeList.add(OtcTransferTypeEnum.transferToOtc.getCode());
			typeList.add(OtcTransferTypeEnum.otcTransferTo.getCode());
			typeList.add(OtcTransferTypeEnum.otcTransferToInnovate.getCode());
			List<OtcUserTransfer> sumOtcTransfer = otcUserTransferMapper.sumOtcTransfer(uid,typeList);
			if(sumOtcTransfer.size() == 0) {
				return map;
			}
			for (OtcUserTransfer otcUserTransfer : sumOtcTransfer) {
				OtcUserTransfer otcTransfer = map.get(otcUserTransfer.getCoinId());
				if(otcTransfer == null) {
					if(otcUserTransfer.getType() == OtcTransferTypeEnum.transferToOtc.getCode()) {
						otcUserTransfer.setAmountOut(otcUserTransfer.getAmount());
						map.put(otcUserTransfer.getCoinId(), otcUserTransfer);
					}
					if(otcUserTransfer.getType() == OtcTransferTypeEnum.otcTransferTo.getCode()) {
						otcUserTransfer.setAmountIn(otcUserTransfer.getAmount());
						map.put(otcUserTransfer.getCoinId(), otcUserTransfer);
					}
					if(otcUserTransfer.getType() == OtcTransferTypeEnum.otcTransferToInnovate.getCode()) {
						otcUserTransfer.setAmountFrozen(otcUserTransfer.getAmount());
						map.put(otcUserTransfer.getCoinId(), otcUserTransfer);
					}
				}else {
					if(otcUserTransfer.getType() == OtcTransferTypeEnum.transferToOtc.getCode()) {
						otcTransfer.setAmountOut(otcUserTransfer.getAmount());
						map.put(otcUserTransfer.getCoinId(), otcTransfer);
					}
					if(otcUserTransfer.getType() == OtcTransferTypeEnum.otcTransferTo.getCode()) {
						otcTransfer.setAmountIn(otcUserTransfer.getAmount());
						map.put(otcUserTransfer.getCoinId(), otcTransfer);
					}
					if(otcUserTransfer.getType() == OtcTransferTypeEnum.otcTransferToInnovate.getCode()) {
						otcTransfer.setAmountFrozen(otcUserTransfer.getAmount());
						map.put(otcUserTransfer.getCoinId(), otcTransfer);
					}
				}
			}
			...
			logger.error("统计otc转账异常",e);
		}
		return null;
	}

	@Override
	public Map<String,BigDecimal> sumUserWalletBalanceLog(Integer uid, List<Integer> types,Date start, Date end) {
		...
		try {
			...
			param.put("end", end);
			List<UserWalletBalanceLog> sumUserWalletBalanceLog = userWalletBalanceLogMapper.sumUserWalletBalanceLog(param);
			if(sumUserWalletBalanceLog != null && sumUserWalletBalanceLog.size() > 0) {
				for (UserWalletBalanceLog u : sumUserWalletBalanceLog) {
					map.put(u.getCoinId().toString() + u.getDirection() + u.getSrcType().toString(), u.getChange());
				}
			}
			return map;
		} catch (Exception e) {
			logger.error("统计资金流水异常",e);
		}
		return null;
	}
