...
public class UserSecurityV2ServiceImpl implements UserSecurityService {
            ...
            switch (action) {
                ...
                case APP_ADD_EMAIL:
                    ...
                    boolean _result = this.validateHelper.mailSendCode(user.getFemail(),validateParam.getPlatform(),
                            validateParam.getBusinessType().getCode(),validateParam.getLocale(),validateParam.getIp(),antiPhingCode);
                    if (!_result) {
                        //邮件发送失败，请稍后再试！
                ...
                case ADD_MAIL:
                    ...
                    boolean result = this.validateHelper.mailSendCode(user.getFemail(), validateParam.getPlatform(),validateParam.getBusinessType().getCode(), validateParam.getLocale(), validateParam.getIp(),antiPhingCode);
                ...
                case SET_ANTI_PHISHING_CODE:
                	break;
                case BIND_TRADEPWD:
                	userInfo.setFtradepwdtime(DateUtils.getPastdayDate(2));
        ...
        switch (action) {
            ...
            case BIND_TRADEPWD:
            	...
            	if (!StringUtils.isEmpty(user.getFtradepassword())) {
            		return Result.failure(1025, "交易密码已设置过！");
				}
                // 绑定交易密码
     ...
     */
    @Override
	public Result validateUserSercuritySetting(FUser user, ValidateParamInfo validateParam) {
		if (user != null) {
			// 如果用户开启了邮箱验证
			if (user.isOpenEmailValidate()) {
				...
				if (!result.getSuccess()) {
					throw new BizException(ErrorCodeEnum.EMAIL_VALID_CODE_ERROR);
				}
			}
			...
			if (user.isOpenPhoneValidate()) {
				Result checkResult = validationCheckHelper.getPhoneCodeCheck(user.getFareacode(),
						...
						validateParam.getPlatform().getCode());
				if (!checkResult.getSuccess()) {
					throw new BizException(ErrorCodeEnum.PHONE_VALID_CODE_ERROR);
				}
			}
			...
			if (user.isOpenGoogleValidate()) {
				...
				if (!checkResult.getSuccess()) {
					throw new BizException(ErrorCodeEnum.GOOGLE_VALID_CODE_ERROR);
				}
			}
		}
		return Result.success();
