...
public class UserLogService {
    ...
    public void upTradeJob() {
    	try {
	        ...
	        for (SystemTradeType tradeType : tradeTypes) {
	            ...
	            Map<String, Object> entrustTotal = fEntrustHistoryMapper.selectTotal(tradeId, pastTime,nowTime);
	            Map<String, Object> entrustPerson = fEntrustHistoryMapper.selectPerson(tradeId,pastTime, nowTime);
     ...
     * 定时器更新交易统计
     */
    public void upTradeJob(Date start,Date end) {
    	try {
    		// 当前时间
            String pastTime = Utils.dateFormatYYYYMMDD(new Timestamp(start.getTime()));
            String nowTime = Utils.dateFormatYYYYMMDD(new Timestamp(end.getTime()));
    		logger.info("开始进行更新交易统计"+nowTime);
	        // 币种
	        List<SystemTradeType> tradeTypes = redisHelper.getTradeTypeList(ActivityConstant.BCAgentId);
	        for (SystemTradeType tradeType : tradeTypes) {
	        	int tradeId = tradeType.getId();
	        	FDayTradeCoin fDayTradeCoin = logActionTx.getFDayTradeCoin(tradeId, nowTime, tradeType.getAgentId());
	        	if(fDayTradeCoin != null) {
	        		continue;
	        	}
	            // 更新买卖单
	            fDayTradeCoin = logActionTx.insertfDayTradeCoin(tradeId, end, tradeType.getAgentId());
	            Map<String, Object> entrustTotal = fEntrustHistoryMapper.selectTotal(tradeId, pastTime,nowTime);
	            Map<String, Object> entrustPerson = fEntrustHistoryMapper.selectPerson(tradeId,pastTime, nowTime);
	            fDayTradeCoin.setFbuy(BigDecimal.valueOf(Double.parseDouble(entrustTotal != null ? entrustTotal.get("sumBuy").toString() : "0")));
	            fDayTradeCoin.setFsell(BigDecimal.valueOf(Double.parseDouble(entrustTotal != null ? entrustTotal.get("sumSell").toString() : "0")));
	            ...
	            fDayTradeCoin.setFbuyperson(Integer.parseInt(entrustPerson != null ? entrustPerson.get("buyCount").toString() : "0"));
	            fDayTradeCoin.setFsellperson(Integer.parseInt(entrustPerson != null ? entrustPerson.get("sellCount").toString() : "0"));
	            fDayTradeCoin.setFupdatetime(end);
	            logActionTx.updatefDayTradeCoin(fDayTradeCoin);
	        }
	        logger.info("统计结束"+nowTime);
		} catch (Exception e) {
			logger.error("更新交易统计异常",e);
		}
    }
