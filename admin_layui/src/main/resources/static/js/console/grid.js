layui.define(['jquery', 'ajax', 'layer'], function (exports) {
    ...
    grid.prototype.button = function () {
        ...
        function buttonEvent($that, othis) {
            ...
            if (lay_target === "dialogInfo") {
                buttonInfoDialog(lay_href, {}, lay_title, lay_width, lay_height, lay_id, othis);
                return;
            }
            if (lay_target === "info") {
            ...
            if (lay_target === "image") {
            	buttonImage(lay_href, lay_title, lay_width, lay_height);
                return;
            }
            if (lay_target === "video") {
            	buttonVideo(lay_href, lay_title, lay_width, lay_height);
                return;
            }
         ...
         * @param callback 成功后回掉函数
         */
        function buttonInfoDialog(lay_href, params, lay_title, lay_width, lay_height, lay_id, othis, callback) {
            var layerIndex = layer.load(2);
            ajax.post(lay_href, params, function (result) {
                var layerOptions = {
                    title: lay_title,
         ...
         * @param lay_height 高度
         */
        function buttonImage(lay_href, lay_title, lay_width, lay_height) {
            var layerIndex = layer.load(2);
            var layerOptions = {
                title: lay_title,
                type: 1,
                skin: 'layui-layer-rim', // 加上边框
                area: [lay_width + 'px', lay_height + 'px'], // 宽高
                content: "<img src='"+lay_href+"' style='width:750px;'>",
                success: function (layero) {
                    if (layero.find(".layui-table").length > 0) {
                        _self.render(layero.find(".layui-layer-content"));
                    }
         ...
         * @param lay_height 高度
         */
        function buttonVideo(lay_href, lay_title, lay_width, lay_height) {
            var layerIndex = layer.load(2);
            var layerOptions = {
                title: lay_title,
                type: 1,
                skin: 'layui-layer-rim', // 加上边框
                area: [lay_width + 'px', lay_height + 'px'], // 宽高
                content: "<video src='"+lay_href+"' style='width:750px;height:500px;' controls='controls'>",
                success: function (layero) {
                    if (layero.find(".layui-table").length > 0) {
                        _self.render(layero.find(".layui-layer-content"));
                    }
...
//        
        ...
        function buttonExport(lay_href,callback) {
            var layerIndex = layer.load(2);
            ajax.get(lay_href,function (result) {
                layer.close(layerIndex);
                if (result.code === 200) {
                    layer.alert(result.msg, {
                        title: "成功",
     ...
     * 简单分页相关
     */
    grid.prototype.simplePage = function () {
        /**
         * 分页初始化
         * @param othis 表格父级对象
         */
        function init(othis) {
            ...
            //判断当前页是否
            var cont = pageEle.parents(".layui-tab-item");
            if(cont.length === 0){
                cont = pageEle.parents(".layui-layer-content");
            }
            var list = cont.find('.layui-table-body').find('tr');
            var curr = pageEle.attr("lay-curr");
            if(list.length < 40){
            	 var layerOptions = {
                         cont: pageEle,
                         pages: 50,
